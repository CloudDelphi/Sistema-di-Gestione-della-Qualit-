...
type
  TFormAuditoriaAgenda2015 = class(TForm)
    pnl1: TPanel;
    btnSair: TBitBtn;
    btnNovo: TBitBtn;
    btnAlterar: TBitBtn;
...
var
  FormAuditoriaAgenda2015: TFormAuditoriaAgenda2015;
...
procedure TFormAuditoriaAgenda2015.AtualizarDados;
begin
   ...
   with cdsAgenda do begin
      ...
      CommandText:= ' SELECT aud_codigo, aud_cod_auditoria, aud_data, aud_horaini, ' +
                    ...
                    ' FROM auditoria_interna_agenda2015' +
...
procedure TFormAuditoriaAgenda2015.Botoes(flag: Boolean);
begin
...
procedure TFormAuditoriaAgenda2015.btnAlterarClick(Sender: TObject);
begin
...
procedure TFormAuditoriaAgenda2015.btnCancelarClick(Sender: TObject);
begin
   LimparCampos(Self);
   PreencherCampos;
   Botoes(True);
...
procedure TFormAuditoriaAgenda2015.btnExcluirClick(Sender: TObject);
var
   sCodAud: string;
begin
//   if (Acesso(cUsuario, 25, 'exclusao') = 1) then begin
      if Application.MessageBox('Confirma a exclusão do registro ?', 'Confirmação', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES then begin
         ...
         with cdsGravar do begin
            Active:= False;
            CommandText:= ' DELETE FROM auditoria_interna_agenda2015' +
...
procedure TFormAuditoriaAgenda2015.btnGravarClick(Sender: TObject);
begin
   sNovoCodigo:= BuscarNovoCodigo('auditoria_interna_agenda2015', 'aud_codigo');

   try
      with cdsGravar do begin
         Active:= False;
         if cOperacao = 'I' then begin
            CommandText:= ' INSERT INTO auditoria_interna_agenda2015(' +
            ...
            CommandText:= ' UPDATE auditoria_interna_agenda2015 SET' +
         ...
         Auditoria('AGENDA AUDITORIA INTERNA 2015', dtData.Text, cOperacao,'');
         AtualizarDados();
...
procedure TFormAuditoriaAgenda2015.btnNovoClick(Sender: TObject);
begin
   cOperacao:= 'I';
   LimparCampos(Self);
   Botoes(False);
...
procedure TFormAuditoriaAgenda2015.btnSairClick(Sender: TObject);
begin
   Self.Close;
end;

procedure TFormAuditoriaAgenda2015.cdsAgendaCalcFields(DataSet: TDataSet);
begin
   cdsAgendaHoraPrevista.AsString:= cdsAgendaaud_horaini.AsString + ' - ' + cdsAgendaaud_horafim.AsString
end;

procedure TFormAuditoriaAgenda2015.dbgFornCellClick(Column: TColumn);
begin
   PreencherCampos();
end;

procedure TFormAuditoriaAgenda2015.FormShow(Sender: TObject);
begin
   HabilitarCampos(False, False, Self, 0);
   AtualizarDados();
   Botoes(True);
...
procedure TFormAuditoriaAgenda2015.PreencherCampos;
begin
   with cdsAgenda do begin
