object FormLaaia: TFormLaaia
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Lista de Aspectos e Avalia'#231#227'o dos Impactos Ambientais'
  ClientHeight = 402
  ClientWidth = 787
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pctLaaia: TJvgPageControl
    Left = 0
    Top = 0
    Width = 787
    Height = 330
    ActivePage = tsCadastro
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    TabStop = False
    OnChange = pctLaaiaChange
    TabStyle.Borders = [fsdLeft, fsdTop, fsdRight, fsdBottom]
    TabStyle.BevelInner = bvNone
    TabStyle.BevelOuter = bvNone
    TabStyle.Bold = False
    TabStyle.BackgrColor = clBtnShadow
    TabStyle.Font.Charset = DEFAULT_CHARSET
    TabStyle.Font.Color = clBtnHighlight
    TabStyle.Font.Height = -11
    TabStyle.Font.Name = 'Arial'
    TabStyle.Font.Style = []
    TabStyle.CaptionHAlign = fhaCenter
    TabStyle.Gradient.Active = False
    TabStyle.Gradient.Orientation = fgdHorizontal
    TabSelectedStyle.Borders = [fsdLeft, fsdTop, fsdRight, fsdBottom]
    TabSelectedStyle.BevelInner = bvNone
    TabSelectedStyle.BevelOuter = bvNone
    TabSelectedStyle.Bold = False
    TabSelectedStyle.BackgrColor = clBtnFace
    TabSelectedStyle.Font.Charset = DEFAULT_CHARSET
    TabSelectedStyle.Font.Color = clBtnText
    TabSelectedStyle.Font.Height = -11
    TabSelectedStyle.Font.Name = 'Arial'
    TabSelectedStyle.Font.Style = []
    TabSelectedStyle.CaptionHAlign = fhaCenter
    TabSelectedStyle.Gradient.Active = False
    TabSelectedStyle.Gradient.Orientation = fgdHorizontal
    Options = [ftoAutoFontDirection, ftoExcludeGlyphs]
    object tsCadastro: TTabSheet
      Caption = 'Cadastro'
      object lbl4: TLabel
        Left = 7
        Top = 48
        Width = 89
        Height = 13
        Caption = 'Aspecto Ambiental'
      end
      object lbl5: TLabel
        Left = 649
        Top = 48
        Width = 109
        Height = 13
        Caption = 'Condi'#231#227'o de Opera'#231#227'o'
      end
      object lbl12: TLabel
        Left = 7
        Top = 139
        Width = 70
        Height = 13
        Caption = 'Prob. ou Freq.'
      end
      object lbl1: TLabel
        Left = 7
        Top = 5
        Width = 54
        Height = 13
        Caption = 'N'#176' Aspecto'
      end
      object lbl2: TLabel
        Left = 76
        Top = 5
        Width = 158
        Height = 13
        Caption = 'Atividades, Produtos ou Servi'#231'os'
      end
      object lbl6: TLabel
        Left = 83
        Top = 139
        Width = 49
        Height = 13
        Caption = 'Gravidade'
      end
      object lbl7: TLabel
        Left = 159
        Top = 139
        Width = 80
        Height = 13
        Caption = 'Grau do Impacto'
      end
      object lbl11: TLabel
        Left = 252
        Top = 139
        Width = 101
        Height = 13
        Caption = 'Controle Operacional'
      end
      object lbl13: TLabel
        Left = 7
        Top = 93
        Width = 143
        Height = 13
        Caption = 'Probabilidade ou Frequ'#234'ncia ?'
      end
      object lblProb: TLabel
        Left = 7
        Top = 111
        Width = 127
        Height = 18
        Caption = 'PROBABILIDADE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lbl15: TLabel
        Left = 189
        Top = 93
        Width = 132
        Height = 13
        Caption = 'Impacto Real ou Potencial ?'
      end
      object lblImp: TLabel
        Left = 189
        Top = 111
        Width = 88
        Height = 18
        Caption = 'POTENCIAL'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lbl17: TLabel
        Left = 8
        Top = 188
        Width = 65
        Height = 13
        Caption = 'Significativo ?'
      end
      object lblSignificativo: TLabel
        Left = 8
        Top = 206
        Width = 30
        Height = 18
        Caption = 'SIM'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lbl19: TLabel
        Left = 338
        Top = 93
        Width = 43
        Height = 13
        Caption = 'Processo'
      end
      object lbl20: TLabel
        Left = 85
        Top = 186
        Width = 67
        Height = 13
        Caption = 'Atualizado em'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object lbl21: TLabel
        Left = 200
        Top = 186
        Width = 66
        Height = 13
        Caption = 'Aprovado por'
      end
      object lbl14: TLabel
        Left = 445
        Top = 186
        Width = 36
        Height = 13
        Caption = 'Eficaz ?'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
        Visible = False
      end
      object lbl16: TLabel
        Left = 665
        Top = 139
        Width = 21
        Height = 13
        Caption = 'PMC'
      end
      object dblAspectos: TDBLookupComboBox
        Left = 7
        Top = 64
        Width = 635
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'asp_codigo'
        ListField = 'asp_descricao'
        ListSource = dsAspectos
        ParentFont = False
        TabOrder = 2
      end
      object edtNumero: TEdit
        Left = 7
        Top = 21
        Width = 60
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object edtAtividade: TEdit
        Left = 76
        Top = 21
        Width = 693
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
      object dblCondicao: TDBLookupComboBox
        Left = 648
        Top = 64
        Width = 121
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_com'
        ListField = 'valo_com'
        ListSource = dsCondicao
        ParentFont = False
        TabOrder = 3
        OnCloseUp = dblCondicaoCloseUp
        OnKeyUp = dblCondicaoKeyUp
      end
      object edtControle: TEdit
        Left = 252
        Top = 155
        Width = 406
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 7
      end
      object dblProcesso: TDBLookupComboBox
        Left = 338
        Top = 109
        Width = 431
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_pro'
        ListField = 'nome_pro'
        ListSource = dsProcessos
        ParentFont = False
        TabOrder = 4
      end
      object edtAprovado: TEdit
        Left = 200
        Top = 202
        Width = 226
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 12
      end
      object dtAtualizacao: TDateEdit
        Left = 85
        Top = 202
        Width = 109
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 11
      end
      object edtGrau: TCurrencyEdit
        Left = 159
        Top = 156
        Width = 80
        Height = 26
        Margins.Left = 5
        Margins.Top = 1
        TabStop = False
        Alignment = taCenter
        DisplayFormat = ' ,0;- ,0'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 10
      end
      object edtGravidade: TCurrencyEdit
        Left = 85
        Top = 156
        Width = 65
        Height = 26
        Hint = 'Valores poss'#237'veis:|1, 3 ou 9|0'
        Margins.Left = 5
        Margins.Top = 1
        Alignment = taCenter
        DisplayFormat = ' ,0;- ,0'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 1
        MinValue = 1.000000000000000000
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 6
        Value = 1.000000000000000000
        OnChange = edtGravidadeChange
        OnKeyPress = edtGravidadeKeyPress
      end
      object edtProb: TCurrencyEdit
        Left = 5
        Top = 156
        Width = 72
        Height = 26
        Hint = 'Valores poss'#237'veis:|1, 3 ou 9|0'
        Margins.Left = 5
        Margins.Top = 1
        Alignment = taCenter
        DisplayFormat = ' ,0;- ,0'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 1
        MinValue = 1.000000000000000000
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 5
        Value = 1.000000000000000000
        OnChange = edtProbChange
        OnKeyPress = edtProbKeyPress
      end
      object edtPMC: TEdit
        Left = 665
        Top = 155
        Width = 77
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 8
      end
      object btnPMC: TBitBtn
        Left = 742
        Top = 149
        Width = 34
        Height = 34
        Hint = 'Abre PMC'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 9
        OnClick = btnPMCClick
      end
      object dblEficaz: TDBLookupComboBox
        Left = 445
        Top = 202
        Width = 88
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_com'
        ListField = 'valo_com'
        ListSource = dsEficaz
        ParentFont = False
        TabOrder = 13
        Visible = False
      end
      object pnlImprimir: TPanel
        Left = 432
        Top = 80
        Width = 383
        Height = 219
        TabOrder = 14
        object lbl24: TLabel
          Left = 15
          Top = 98
          Width = 43
          Height = 13
          Caption = 'Processo'
        end
        object pnl2: TPanel
          Left = 1
          Top = 146
          Width = 381
          Height = 72
          Align = alBottom
          TabOrder = 4
          object btnSairImp: TBitBtn
            Left = 307
            Top = 1
            Width = 73
            Height = 70
            Hint = 'Clique para sair'
            Align = alRight
            Caption = 'Sair'
            DoubleBuffered = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentDoubleBuffered = False
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 2
            OnClick = btnSairImpClick
          end
          object btnVideo: TBitBtn
            Left = 1
            Top = 1
            Width = 73
            Height = 70
            Hint = 'Clique para imprimir'
            Align = alLeft
            Caption = '&V'#237'deo'
            DoubleBuffered = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentDoubleBuffered = False
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 0
            OnClick = btnVideoClick
          end
          object btnImpressora: TBitBtn
            Left = 74
            Top = 1
            Width = 73
            Height = 70
            Hint = 'Clique para imprimir'
            Align = alLeft
            Caption = '&Impressora'
            DoubleBuffered = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentDoubleBuffered = False
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 1
            OnClick = btnImpressoraClick
          end
        end
        object pnl3: TPanel
          Left = 1
          Top = 1
          Width = 381
          Height = 29
          Align = alTop
          Alignment = taLeftJustify
          Caption = ' Filtro de Impress'#227'o'
          Color = clGradientActiveCaption
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object rgOrdemImpressao: TRadioGroup
          Left = 15
          Top = 37
          Width = 356
          Height = 59
          Caption = 'Ordem de Impress'#227'o'
          Columns = 2
          Items.Strings = (
            'Num. Aspecto'
            'Atividade, Produtos ou Servi'#231'os')
          TabOrder = 1
        end
        object dblProcessoImp: TDBLookupComboBox
          Left = 15
          Top = 114
          Width = 292
          Height = 26
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Tahoma'
          Font.Style = []
          KeyField = 'codi_pro'
          ListField = 'nome_pro'
          ListSource = dsProcessos
          ParentFont = False
          TabOrder = 2
        end
        object chkTodosProc: TCheckBox
          Left = 310
          Top = 118
          Width = 62
          Height = 17
          Caption = 'TODOS'
          TabOrder = 3
          OnClick = chkTodosProcClick
        end
      end
      object dbgExcel: TDBGrid
        Left = 535
        Top = -5
        Width = 204
        Height = 74
        DataSource = dsExcel
        TabOrder = 15
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Visible = False
      end
    end
    object tsImpactos: TTabSheet
      Caption = 'Impactos Ambientais'
      ImageIndex = 3
      OnShow = tsLeisShow
      object lbl3: TLabel
        Left = 7
        Top = 56
        Width = 89
        Height = 13
        Caption = 'Impacto Ambiental'
      end
      object lbl25: TLabel
        Left = 7
        Top = 5
        Width = 54
        Height = 13
        Caption = 'N'#176' Aspecto'
      end
      object lbl26: TLabel
        Left = 76
        Top = 5
        Width = 158
        Height = 13
        Caption = 'Atividades, Produtos ou Servi'#231'os'
      end
      object dblImpactos: TDBLookupComboBox
        Left = 7
        Top = 72
        Width = 359
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'imp_codigo'
        ListField = 'imp_descricao'
        ListSource = dsImpactos
        ParentFont = False
        TabOrder = 4
      end
      object btnGravarImpacto: TBitBtn
        Left = 599
        Top = 50
        Width = 82
        Height = 51
        Hint = 'Insere a Lei'
        Caption = 'Inserir'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnGravarImpactoClick
      end
      object btnExcluirImpacto: TBitBtn
        Left = 687
        Top = 50
        Width = 82
        Height = 51
        Hint = 'Exclui a Lei'
        Caption = 'Excluir'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btnExcluirImpactoClick
      end
      object dbgLaaiaImpactos: TDBGrid
        Left = 7
        Top = 104
        Width = 762
        Height = 195
        Hint = 'Selecione o registro para alterar'
        DataSource = dsLaaiaImpactos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 5
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnKeyUp = dbgLaaiaKeyUp
        Columns = <
          item
            Expanded = False
            FieldName = 'DescImpacto'
            Title.Caption = 'Impacto Ambiental'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 630
            Visible = True
          end>
      end
      object edtNumAspectoImp: TEdit
        Left = 7
        Top = 21
        Width = 60
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object edtAtividadeImp: TEdit
        Left = 76
        Top = 21
        Width = 693
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
    end
    object tsLeis: TTabSheet
      Caption = 'Leis'
      ImageIndex = 2
      OnShow = tsLeisShow
      object lbl8: TLabel
        Left = 7
        Top = 59
        Width = 62
        Height = 13
        Caption = 'Identifica'#231#227'o'
      end
      object lbl18: TLabel
        Left = 121
        Top = 59
        Width = 108
        Height = 13
        Caption = 'Descri'#231#227'o do Requisito'
      end
      object lbl22: TLabel
        Left = 7
        Top = 5
        Width = 54
        Height = 13
        Caption = 'N'#176' Aspecto'
      end
      object lbl23: TLabel
        Left = 76
        Top = 5
        Width = 158
        Height = 13
        Caption = 'Atividades, Produtos ou Servi'#231'os'
      end
      object edtIdentLei: TEdit
        Left = 7
        Top = 75
        Width = 62
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        OnExit = edtIdentLeiExit
      end
      object edtDescLei: TEdit
        Left = 121
        Top = 75
        Width = 472
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
      end
      object dbgLeis: TDBGrid
        Left = 7
        Top = 104
        Width = 762
        Height = 195
        Hint = 'Selecione o registro para alterar'
        DataSource = dsLeis
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 7
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnKeyUp = dbgLaaiaKeyUp
        Columns = <
          item
            Expanded = False
            FieldName = 'lir_identificacao'
            Title.Caption = 'Identifica'#231#227'o'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DescRequisito'
            Title.Caption = 'Descri'#231#227'o do requisito'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 630
            Visible = True
          end>
      end
      object btn1: TBitBtn
        Left = 78
        Top = 67
        Width = 34
        Height = 34
        Hint = 'Pesquisar Leis'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 4
        OnClick = btn1Click
      end
      object btnExcluirLei: TBitBtn
        Left = 687
        Top = 50
        Width = 82
        Height = 51
        Hint = 'Exclui a Lei'
        Caption = 'Excluir'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btnExcluirLeiClick
      end
      object btnInserirLei: TBitBtn
        Left = 599
        Top = 50
        Width = 82
        Height = 51
        Hint = 'Insere a Lei'
        Caption = 'Inserir'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnInserirLeiClick
      end
      object edtNumAspectoLei: TEdit
        Left = 7
        Top = 21
        Width = 60
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object edtAtividadeLei: TEdit
        Left = 76
        Top = 21
        Width = 693
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
    end
    object tsPesquisa: TTabSheet
      Caption = 'Pesquisa'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageIndex = 1
      ParentFont = False
      object lbl9: TLabel
        Left = 3
        Top = 13
        Width = 42
        Height = 13
        Caption = 'Pesquisa'
      end
      object lbl10: TLabel
        Left = 327
        Top = 13
        Width = 65
        Height = 13
        Caption = 'Pesquisar por'
      end
      object dbgLaaia: TDBGrid
        Left = 3
        Top = 60
        Width = 775
        Height = 239
        Hint = 'Selecione o registro para alterar'
        DataSource = dsLaaia
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnCellClick = dbgLaaiaCellClick
        OnDblClick = dbgLaaiaDblClick
        OnKeyUp = dbgLaaiaKeyUp
        Columns = <
          item
            Expanded = False
            FieldName = 'lai_numAspecto'
            Title.Caption = 'N. Asp.'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 45
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'lai_atividade'
            Title.Caption = 'Atividades, Produtos ou Servi'#231'os'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 320
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DescAspecto'
            Title.Caption = 'Aspecto Ambiental'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 280
            Visible = True
          end>
      end
      object edtValor: TEdit
        Left = 3
        Top = 32
        Width = 318
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnChange = edtValorChange
      end
      object cbCampo: TComboBox
        Left = 327
        Top = 32
        Width = 298
        Height = 26
        Style = csDropDownList
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ItemIndex = 0
        ParentFont = False
        TabOrder = 1
        Text = 'Atividades, Produtos ou Servi'#231'os'
        Items.Strings = (
          'Atividades, Produtos ou Servi'#231'os')
      end
    end
  end
  object pnl1: TPanel
    Left = 0
    Top = 330
    Width = 787
    Height = 72
    Align = alBottom
    TabOrder = 1
    object btnNovo: TBitBtn
      Left = 1
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para adicionar um novo registro'
      Align = alLeft
      Caption = '&Novo'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = btnNovoClick
    end
    object btnGravar: TBitBtn
      Left = 220
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para gravar o registro'
      Align = alLeft
      Caption = '&Gravar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = btnGravarClick
    end
    object btnExcluir: TBitBtn
      Left = 147
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para excluir um registro'
      Align = alLeft
      Caption = '&Excluir'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = btnExcluirClick
    end
    object btnCancelar: TBitBtn
      Left = 293
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para cancelar a opera'#231#227'o'
      Align = alLeft
      Caption = '&Cancelar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnClick = btnCancelarClick
    end
    object btnImprimir: TBitBtn
      Left = 366
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para imprimir'
      Align = alLeft
      Caption = '&Imprimir'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnClick = btnImprimirClick
    end
    object btnSair: TBitBtn
      Left = 713
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para sair'
      Align = alRight
      Caption = 'Sair'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = btnSairClick
    end
    object btnAlterar: TBitBtn
      Left = 74
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para alterar um registro'
      Align = alLeft
      Caption = '&Alterar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFDFDFDAEACAB757372817E7ED6D5D5FFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = btnAlterarClick
    end
    object pnlNavegacao: TPanel
      Left = 543
      Top = 18
      Width = 161
      Height = 40
      TabOrder = 7
      object btnPrimeiro: TBitBtn
        Left = 1
        Top = 1
        Width = 40
        Height = 38
        Hint = 'Primeiro registro'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = btnPrimeiroClick
      end
      object btnUltimo: TBitBtn
        Left = 121
        Top = 1
        Width = 40
        Height = 38
        Hint = #218'ltimo registro'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btnUltimoClick
      end
      object btnProximo: TBitBtn
        Left = 81
        Top = 1
        Width = 40
        Height = 38
        Hint = 'Pr'#243'ximo registro'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnProximoClick
      end
      object btnAnterior: TBitBtn
        Left = 41
        Top = 1
        Width = 40
        Height = 38
        Hint = 'Registro anterior'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        OnClick = btnAnteriorClick
      end
    end
    object btnExcel: TBitBtn
      Left = 439
      Top = 1
      Width = 95
      Height = 70
      Hint = 'Clique para exportar as informa'#231#245'es para o Excel'
      Align = alLeft
      Caption = '&Exportar Excel'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        EFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 8
      OnClick = btnExcelClick
    end
  end
  object zqryLaaia: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT lai_numAspecto, lai_atividade, lai_aspecto, lai_condicao,' +
        ' lai_processo, lai_frequencia, '
      
        'lai_gravidade, lai_controle, lai_pmc, lai_atualizacao, lai_aprov' +
        'ado, lai_eficaz, lai_impacto'
      'FROM Laaia'
      'ORDER BY lai_numAspecto')
    Params = <>
    Left = 216
    Top = 224
  end
  object dspLaaia: TDataSetProvider
    DataSet = zqryLaaia
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 256
    Top = 224
  end
  object cdsLaaia: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspLaaia'
    Left = 296
    Top = 224
    object cdsLaaialai_numAspecto: TLargeintField
      FieldName = 'lai_numAspecto'
      Required = True
    end
    object cdsLaaialai_atividade: TWideStringField
      FieldName = 'lai_atividade'
      Size = 80
    end
    object cdsLaaialai_aspecto: TLargeintField
      FieldName = 'lai_aspecto'
    end
    object cdsLaaialai_condicao: TLargeintField
      FieldName = 'lai_condicao'
    end
    object cdsLaaialai_processo: TLargeintField
      FieldName = 'lai_processo'
    end
    object cdsLaaialai_frequencia: TLargeintField
      FieldName = 'lai_frequencia'
    end
    object cdsLaaialai_gravidade: TLargeintField
      FieldName = 'lai_gravidade'
    end
    object cdsLaaialai_controle: TWideStringField
      FieldName = 'lai_controle'
      Size = 60
    end
    object cdsLaaialai_pmc: TWideStringField
      FieldName = 'lai_pmc'
      Size = 15
    end
    object cdsLaaialai_atualizacao: TDateField
      FieldName = 'lai_atualizacao'
    end
    object cdsLaaialai_aprovado: TWideStringField
      FieldName = 'lai_aprovado'
      Size = 50
    end
    object cdsLaaialai_eficaz: TWideStringField
      FieldName = 'lai_eficaz'
      Size = 3
    end
    object cdsLaaiaDescAspecto: TStringField
      FieldKind = fkLookup
      FieldName = 'DescAspecto'
      LookupDataSet = cdsAspectos
      LookupKeyFields = 'asp_codigo'
      LookupResultField = 'asp_descricao'
      KeyFields = 'lai_aspecto'
      Size = 80
      Lookup = True
    end
    object cdsLaaiaDescImpacto: TStringField
      FieldKind = fkLookup
      FieldName = 'DescImpacto'
      LookupDataSet = cdsImpactos
      LookupKeyFields = 'imp_codigo'
      LookupResultField = 'imp_descricao'
      KeyFields = 'lai_impacto'
      Size = 80
      Lookup = True
    end
    object cdsLaaialai_impacto: TLargeintField
      FieldName = 'lai_impacto'
      ReadOnly = True
    end
  end
  object dsLaaia: TDataSource
    DataSet = cdsLaaia
    Left = 336
    Top = 224
  end
  object zqryAspectos: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT asp_codigo, asp_descricao '
      'FROM aspectos_ambientais'
      'ORDER BY asp_descricao')
    Params = <>
    Left = 32
    Top = 224
  end
  object dspAspectos: TDataSetProvider
    DataSet = zqryAspectos
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 72
    Top = 224
  end
  object cdsAspectos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspAspectos'
    Left = 112
    Top = 224
    object cdsAspectosasp_codigo: TLargeintField
      FieldName = 'asp_codigo'
      Required = True
    end
    object cdsAspectosasp_descricao: TWideStringField
      FieldName = 'asp_descricao'
      Size = 80
    end
  end
  object dsAspectos: TDataSource
    DataSet = cdsAspectos
    Left = 152
    Top = 224
  end
  object zqryImpactos: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT imp_codigo, imp_descricao '
      'FROM impactos_ambientais'
      'ORDER BY imp_descricao')
    Params = <>
    Left = 32
    Top = 272
  end
  object dspImpactos: TDataSetProvider
    DataSet = zqryImpactos
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 72
    Top = 272
  end
  object cdsImpactos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspImpactos'
    Left = 112
    Top = 272
    object cdsImpactosimp_codigo: TLargeintField
      FieldName = 'imp_codigo'
      Required = True
    end
    object cdsImpactosimp_descricao: TWideStringField
      FieldName = 'imp_descricao'
      Size = 80
    end
  end
  object dsImpactos: TDataSource
    DataSet = cdsImpactos
    Left = 152
    Top = 272
  end
  object zqryCondicao: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_com, valo_com FROM tabela_combos')
    Params = <>
    Left = 616
    Top = 272
  end
  object dspCondicao: TDataSetProvider
    DataSet = zqryCondicao
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 656
    Top = 272
  end
  object cdsCondicao: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspCondicao'
    Left = 696
    Top = 272
    object cdsCondicaocodi_com: TLargeintField
      FieldName = 'codi_com'
      Required = True
    end
    object cdsCondicaovalo_com: TWideStringField
      FieldName = 'valo_com'
      Size = 50
    end
  end
  object dsCondicao: TDataSource
    DataSet = cdsCondicao
    Left = 736
    Top = 272
  end
  object zqryProcessos: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      '')
    Params = <>
    Left = 216
    Top = 272
  end
  object dspProcessos: TDataSetProvider
    DataSet = zqryProcessos
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 256
    Top = 272
  end
  object cdsProcessos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspProcessos'
    Left = 296
    Top = 272
  end
  object dsProcessos: TDataSource
    DataSet = cdsProcessos
    Left = 336
    Top = 272
  end
  object zqryGravar: TZQuery
    Connection = dm.Conexao
    Params = <>
    Left = 480
    Top = 24
  end
  object dspGravar: TDataSetProvider
    DataSet = zqryGravar
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 520
    Top = 24
  end
  object cdsGravar: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspGravar'
    Left = 560
    Top = 24
  end
  object zqryImprimir: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT IA.imp_descricao, L.lai_numAspecto, lai_atividade, lai_as' +
        'pecto, lai_impacto, lai_condicao, lai_processo, lai_frequencia,'
      
        'lai_gravidade, lai_controle, lai_atualizacao, lai_aprovado, lai_' +
        'eficaz, nome_pro'
      'FROM Laaia L'
      
        'INNER JOIN laaia_impactos I ON I.lai_numaspecto = L.lai_numAspec' +
        'to'
      
        'INNER JOIN impactos_ambientais IA ON IA.imp_codigo = I.lai_codim' +
        'pacto'
      'INNER JOIN Processos P ON P.codi_pro = L.lai_processo'
      'ORDER BY L.lai_numAspecto')
    Params = <>
    Left = 480
    Top = 72
  end
  object dspImprimir: TDataSetProvider
    DataSet = zqryImprimir
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 520
    Top = 72
  end
  object cdsImprimir: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspImprimir'
    OnCalcFields = cdsImprimirCalcFields
    Left = 560
    Top = 72
    object cdsImprimirlai_numaspecto: TLargeintField
      FieldName = 'lai_numaspecto'
      Required = True
    end
    object cdsImprimirlai_atividade: TWideStringField
      FieldName = 'lai_atividade'
      Size = 80
    end
    object cdsImprimirlai_aspecto: TLargeintField
      FieldName = 'lai_aspecto'
    end
    object cdsImprimirlai_impacto: TLargeintField
      FieldName = 'lai_impacto'
    end
    object cdsImprimirlai_condicao: TLargeintField
      FieldName = 'lai_condicao'
    end
    object cdsImprimirlai_processo: TLargeintField
      FieldName = 'lai_processo'
    end
    object cdsImprimirlai_frequencia: TLargeintField
      FieldName = 'lai_frequencia'
    end
    object cdsImprimirlai_gravidade: TLargeintField
      FieldName = 'lai_gravidade'
    end
    object cdsImprimirlai_controle: TWideStringField
      FieldName = 'lai_controle'
      Size = 60
    end
    object cdsImprimirlai_atualizacao: TDateField
      FieldName = 'lai_atualizacao'
    end
    object cdsImprimirlai_aprovado: TWideStringField
      FieldName = 'lai_aprovado'
      Size = 50
    end
    object cdsImprimirlai_eficaz: TWideStringField
      FieldName = 'lai_eficaz'
      Size = 3
    end
    object cdsImprimirnome_pro: TWideStringField
      FieldName = 'nome_pro'
      Size = 45
    end
    object cdsImprimirDescProcesso: TStringField
      FieldKind = fkLookup
      FieldName = 'DescProcesso'
      LookupDataSet = cdsProcessos
      LookupKeyFields = 'codi_pro'
      LookupResultField = 'nome_pro'
      KeyFields = 'lai_processo'
      Size = 80
      Lookup = True
    end
    object cdsImprimirDescAspecto: TStringField
      FieldKind = fkLookup
      FieldName = 'DescAspecto'
      LookupDataSet = cdsAspectos
      LookupKeyFields = 'asp_codigo'
      LookupResultField = 'asp_descricao'
      KeyFields = 'lai_aspecto'
      Size = 80
      Lookup = True
    end
    object cdsImprimirDescImpacto: TStringField
      FieldKind = fkLookup
      FieldName = 'DescImpacto'
      LookupDataSet = cdsImpactos
      LookupKeyFields = 'imp_codigo'
      LookupResultField = 'imp_descricao'
      KeyFields = 'lai_impacto'
      Size = 80
      Lookup = True
    end
    object cdsImprimirDescCondicao: TStringField
      FieldKind = fkLookup
      FieldName = 'DescCondicao'
      LookupDataSet = cdsCondicao
      LookupKeyFields = 'codi_com'
      LookupResultField = 'valo_com'
      KeyFields = 'lai_condicao'
      Lookup = True
    end
    object cdsImprimirDescProbabilidade: TStringField
      FieldKind = fkCalculated
      FieldName = 'DescProbabilidade'
      Size = 30
      Calculated = True
    end
    object cdsImprimirDescReal: TStringField
      FieldKind = fkCalculated
      FieldName = 'DescReal'
      Size = 10
      Calculated = True
    end
    object cdsImprimirGrauImpacto: TIntegerField
      FieldKind = fkCalculated
      FieldName = 'GrauImpacto'
      Calculated = True
    end
    object cdsImprimirSignificativo: TStringField
      FieldKind = fkCalculated
      FieldName = 'Significativo'
      Size = 3
      Calculated = True
    end
    object cdsImprimirimp_descricao: TWideStringField
      FieldName = 'imp_descricao'
      Size = 80
    end
  end
  object zqryLeis: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT lai_numAspecto, lir_identificacao'
      'FROM laaia_lei')
    Params = <>
    Left = 424
    Top = 288
  end
  object dspLeis: TDataSetProvider
    DataSet = zqryLeis
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 464
    Top = 288
  end
  object cdsLeis: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspLeis'
    Left = 504
    Top = 288
    object cdsLeislai_numAspecto: TLargeintField
      FieldName = 'lai_numAspecto'
      Required = True
    end
    object cdsLeislir_identificacao: TWideStringField
      FieldName = 'lir_identificacao'
      Required = True
      Size = 6
    end
    object cdsLeisDescRequisito: TStringField
      FieldKind = fkLookup
      FieldName = 'DescRequisito'
      LookupDataSet = cdsLira
      LookupKeyFields = 'lir_identificacao'
      LookupResultField = 'lir_descricao'
      KeyFields = 'lir_identificacao'
      Size = 80
      Lookup = True
    end
  end
  object dsLeis: TDataSource
    DataSet = cdsLeis
    Left = 544
    Top = 288
  end
  object zqryLira: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT lir_codigo, lir_identificacao, lir_descricao'
      'FROM lira')
    Params = <>
    Left = 424
    Top = 240
  end
  object dspLira: TDataSetProvider
    DataSet = zqryLira
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 464
    Top = 240
  end
  object cdsLira: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspLira'
    Left = 504
    Top = 240
    object cdsLiralir_codigo: TLargeintField
      FieldName = 'lir_codigo'
      Required = True
    end
    object cdsLiralir_identificacao: TWideStringField
      FieldName = 'lir_identificacao'
      Size = 6
    end
    object cdsLiralir_descricao: TWideStringField
      FieldName = 'lir_descricao'
      Size = 80
    end
  end
  object dsLira: TDataSource
    DataSet = cdsLira
    Left = 544
    Top = 240
  end
  object zqryEficaz: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_com, valo_com FROM tabela_combos')
    Params = <>
    Left = 616
    Top = 224
  end
  object dspEficaz: TDataSetProvider
    DataSet = zqryEficaz
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 656
    Top = 224
  end
  object cdsEficaz: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspEficaz'
    Left = 696
    Top = 224
    object LargeintField2: TLargeintField
      FieldName = 'codi_com'
      Required = True
    end
    object WideStringField2: TWideStringField
      FieldName = 'valo_com'
      Size = 50
    end
  end
  object dsEficaz: TDataSource
    DataSet = cdsEficaz
    Left = 736
    Top = 224
  end
  object frxReport1: TfrxReport
    Version = '4.10.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40548.430550960600000000
    ReportOptions.LastChange = 42748.542885081020000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      'begin'
      ''
      'end.')
    Left = 480
    Top = 144
    Datasets = <
      item
        DataSet = frxDBDSLaaia
        DataSetName = 'frxDBDSLaaia'
      end
      item
        DataSet = frxDBLeis
        DataSetName = 'frxDBLeis'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        Height = 163.842610000000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Memo4: TfrxMemoView
          Top = 21.267780000000000000
          Width = 1046.929810000000000000
          Height = 34.015770000000000000
          ShowHint = False
          AutoWidth = True
          DataSetName = 'cdsParametroRelatorio'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -21
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            
              'L.A.A.I.A. - LISTA DE ASPECTOS E AVALIA'#199#195'O DOS IMPACTOS AMBIENTA' +
              'IS')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Width = 1046.929810000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo6: TfrxMemoView
          Left = 0.559060000000000000
          Top = 69.921259840000000000
          Width = 44.976348660000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'N'#250'm. Aspecto')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          Left = 45.094488190000000000
          Top = 69.921259840000000000
          Width = 185.196823540000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Atividades, Produtos ou Servi'#231'os')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          Left = 230.110236220000000000
          Top = 69.921259840000000000
          Width = 272.126013540000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Aspecto Ambiental')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          Left = 502.701300000000000000
          Top = 69.921259842519710000
          Width = 75.590512130000000000
          Height = 94.488188976378000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Condi'#231#227'o de Opera'#231#227'o')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo3: TfrxMemoView
          Left = 578.260360000000000000
          Top = 69.921259840000000000
          Width = 79.370078740000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Probabilidade ou Frequ'#234'ncia')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          Left = 657.638220000000000000
          Top = 69.921259840000000000
          Width = 56.692898740000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Impacto Real ou Potencial')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          Left = 714.110700000000000000
          Top = 69.811070000000000000
          Width = 22.677128740000000000
          Height = 94.488250000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Prob. ou Freq.')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          Left = 736.787880000000000000
          Top = 69.811070000000000000
          Width = 22.677128740000000000
          Height = 94.488250000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Gravidade')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          Left = 759.244590000000000000
          Top = 69.811070000000000000
          Width = 22.677128740000000000
          Height = 94.488250000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Grau do Impacto')
          ParentFont = False
          Rotation = 90
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          Left = 782.142240000000000000
          Top = 69.811070000000000000
          Width = 79.370078740000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Significativo ?')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo24: TfrxMemoView
          Left = 860.732840000000000000
          Top = 70.031540000000010000
          Width = 181.417388740000000000
          Height = 94.488188980000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Controle Operacional')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object Detalhe: TfrxMasterData
        Height = 18.897588980000000000
        Top = 389.291590000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBDSLaaia
        DataSetName = 'frxDBDSLaaia'
        RowCount = 0
        object Memo10: TfrxMemoView
          Width = 1042.015748030000000000
          Height = 18.897588980000000000
          ShowHint = False
          DataField = 'imp_descricao'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDSLaaia."imp_descricao"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 52.913420000000000000
        Top = 468.661720000000000000
        Width = 1046.929810000000000000
        object Picture1: TfrxPictureView
          Left = 993.795920000000000000
          Top = 7.559059999999988000
          Width = 34.015770000000000000
          Height = 34.015770000000000000
          ShowHint = False
          Picture.Data = {
            
            D9}
          Stretched = False
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo9: TfrxMemoView
          Left = 3.779530000000000000
          Top = 30.236239999999900000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'P'#225'gina: [Page#] de [TotalPages#]')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = 3.779530000000000000
          Top = 26.456709999999870000
          Width = 978.898270000000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
      end
      object Processo: TfrxGroupHeader
        Height = 22.677180000000000000
        Top = 241.889920000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBDSLaaia."DescProcesso"'
        object Memo23: TfrxMemoView
          Width = 1042.393700790000000000
          Height = 22.677180000000000000
          ShowHint = False
          Color = clBtnShadow
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDSLaaia."DescProcesso"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object GroupHeader1: TfrxGroupHeader
        Height = 79.370068980000000000
        Top = 287.244280000000000000
        Width = 1046.929810000000000000
        Condition = 'frxDBDSLaaia."lai_numAspecto"'
        object Memo29: TfrxMemoView
          Left = -0.220470000000000000
          Width = 44.976368190000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'lai_numAspecto'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."lai_numAspecto"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          Left = 44.976377952755900000
          Width = 185.196823540000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'lai_atividade'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDSLaaia."lai_atividade"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          Left = 229.952758350000000000
          Width = 272.126013540000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'DescAspecto'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDSLaaia."DescAspecto"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          Left = 502.102877240000000000
          Width = 75.590502360000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'DescCondicao'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."DescCondicao"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          Left = 577.511811023622000000
          Width = 79.370054330000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'DescProbabilidade'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."DescProbabilidade"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          Left = 657.417325280000000000
          Width = 56.692874330000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'DescReal'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."DescReal"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          Left = 713.732285910000000000
          Width = 22.677165350000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'lai_frequencia'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."lai_frequencia"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo37: TfrxMemoView
          Left = 736.409451260000000000
          Width = 22.677165350000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'lai_gravidade'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."lai_gravidade"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo38: TfrxMemoView
          Left = 759.086616610000000000
          Width = 22.677165350000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'GrauImpacto'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."GrauImpacto"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo39: TfrxMemoView
          Left = 781.763781970000000000
          Width = 79.370078740000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'Significativo'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."Significativo"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo40: TfrxMemoView
          Left = 861.377955200000000000
          Width = 180.661417320000000000
          Height = 60.472480000000000000
          ShowHint = False
          DataField = 'lai_controle'
          DataSet = frxDBDSLaaia
          DataSetName = 'frxDBDSLaaia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDSLaaia."lai_controle"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          Top = 60.472480000000010000
          Width = 1042.015748030000000000
          Height = 18.897588980000000000
          ShowHint = False
          Color = cl3DLight
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Impactos Ambientais')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
    object Page2: TfrxReportPage
      Visible = False
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object MasterData1: TfrxMasterData
        Height = 22.677180000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        DataSet = frxDBLeis
        DataSetName = 'frxDBLeis'
        RowCount = 0
        object Memo1: TfrxMemoView
          Width = 94.488188980000000000
          Height = 22.677180000000000000
          ShowHint = False
          DataField = 'lir_identificacao'
          DataSet = frxDBLeis
          DataSetName = 'frxDBLeis'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBLeis."lir_identificacao"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          Left = 94.267780000000000000
          Width = 520.818897640000000000
          Height = 22.677180000000000000
          ShowHint = False
          DataField = 'DescRequisito'
          DataSet = frxDBLeis
          DataSetName = 'frxDBLeis'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBLeis."DescRequisito"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object frxDBDSLaaia: TfrxDBDataset
    UserName = 'frxDBDSLaaia'
    CloseDataSource = False
    FieldAliases.Strings = (
      'lai_numaspecto=lai_numAspecto'
      'lai_atividade=lai_atividade'
      'lai_aspecto=lai_aspecto'
      'lai_impacto=lai_impacto'
      'lai_condicao=lai_condicao'
      'lai_processo=lai_processo'
      'lai_frequencia=lai_frequencia'
      'lai_gravidade=lai_gravidade'
      'lai_controle=lai_controle'
      'lai_atualizacao=lai_atualizacao'
      'lai_aprovado=lai_aprovado'
      'lai_eficaz=lai_eficaz'
      'nome_pro=nome_pro'
      'DescProcesso=DescProcesso'
      'DescAspecto=DescAspecto'
      'DescImpacto=DescImpacto'
      'DescCondicao=DescCondicao'
      'DescProbabilidade=DescProbabilidade'
      'DescReal=DescReal'
      'GrauImpacto=GrauImpacto'
      'Significativo=Significativo'
      'imp_descricao=imp_descricao')
    DataSet = cdsImprimir
    BCDToCurrency = False
    Left = 576
    Top = 144
  end
  object frxDBLeis: TfrxDBDataset
    UserName = 'frxDBLeis'
    CloseDataSource = False
    FieldAliases.Strings = (
      'lai_numAspecto=lai_numAspecto'
      'lir_identificacao=lir_identificacao'
      'DescRequisito=DescRequisito')
    DataSet = cdsLeis
    BCDToCurrency = False
    Left = 632
    Top = 144
  end
  object zqryLaaiaImpactos: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT lai_numAspecto, lai_codImpacto'
      'FROM laaia_impactos')
    Params = <>
    Left = 424
    Top = 192
  end
  object dspLaaiaImpactos: TDataSetProvider
    DataSet = zqryLaaiaImpactos
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 464
    Top = 192
  end
  object cdsLaaiaImpactos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspLaaiaImpactos'
    Left = 504
    Top = 192
    object cdsLaaiaImpactoslai_numAspecto: TLargeintField
      FieldName = 'lai_numAspecto'
      Required = True
    end
    object cdsLaaiaImpactoslai_codImpacto: TLargeintField
      FieldName = 'lai_codImpacto'
      Required = True
    end
    object cdsLaaiaImpactosDescImpacto: TStringField
      FieldKind = fkLookup
      FieldName = 'DescImpacto'
      LookupDataSet = cdsImpactos
      LookupKeyFields = 'imp_codigo'
      LookupResultField = 'imp_descricao'
      KeyFields = 'lai_codImpacto'
      Size = 80
      Lookup = True
    end
  end
  object dsLaaiaImpactos: TDataSource
    DataSet = cdsLaaiaImpactos
    Left = 544
    Top = 192
  end
  object zqryExcel: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT L.lai_numAspecto as "Num. Impacto", lai_atividade as "Ati' +
        'vidades, Produtos ou Servi'#231'os", A.asp_descricao as "Aspecto Ambi' +
        'ental", '
      'TC.valo_com as "Condi'#231#227'o de Opera'#231#227'o", '
      'CASE WHEN lai_condicao = 1 THEN '#39'FREQU'#202'NCIA'#39
      '     WHEN lai_condicao = 2 THEN '#39'FREQU'#202'NCIA'#39
      'ELSE '#39'PROBABILIDADE'#39
      'END as "Frequencia ou Probabildade",'
      'CASE WHEN lai_condicao = 1 THEN '#39'REAL'#39
      '     WHEN lai_condicao = 2 THEN '#39'REAL'#39
      'ELSE '#39'POTENCIAL'#39
      'END as "Impacto Real ou Potencial",'
      
        'lai_frequencia as "Frequencia", lai_gravidade as "Gravidade", la' +
        'i_frequencia * lai_gravidade as "Grau do Impacto",'
      'CASE WHEN lai_frequencia * lai_gravidade <= 9 THEN '#39'N'#195'O'#39
      'ELSE '#39'SIM'#39
      'END as "Significativo",'
      
        'lai_controle as "Controle Operacional", IA.imp_descricao as "Imp' +
        'acto Ambiental", P.nome_pro as "Processo"'
      'FROM Laaia L'
      
        'LEFT JOIN laaia_impactos I ON I.lai_numaspecto = L.lai_numAspect' +
        'o'
      
        'LEFT JOIN impactos_ambientais IA ON IA.imp_codigo = I.lai_codimp' +
        'acto'
      'INNER JOIN processos P ON P.codi_pro = lai_processo '
      
        'INNER JOIN tabela_combos TC ON TC.tipo_com = 11 AND TC.codi_com ' +
        '= lai_condicao'
      'INNER JOIN aspectos_ambientais A ON A.asp_codigo = L.lai_aspecto'
      'ORDER BY L.lai_numAspecto')
    Params = <>
    Left = 616
    Top = 24
  end
  object dspExcel: TDataSetProvider
    DataSet = zqryExcel
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 656
    Top = 24
  end
  object dsExcel: TDataSource
    DataSet = cdsExcel
    Left = 736
    Top = 24
  end
  object cdsExcel: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspExcel'
    OnCalcFields = cdsExcelCalcFields
    Left = 696
    Top = 24
  end
end
