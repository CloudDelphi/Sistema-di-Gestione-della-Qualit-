object FormResumoInd: TFormResumoInd
  Left = 0
  Top = 0
  BorderStyle = bsSingle
  Caption = 'Resumo de Indicadores'
  ClientHeight = 254
  ClientWidth = 426
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object lbl1: TLabel
    Left = 17
    Top = 21
    Width = 72
    Height = 13
    Caption = 'M'#234's/Ano Inicial'
    Color = clBackground
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object lbl2: TLabel
    Left = 98
    Top = 47
    Width = 6
    Height = 13
    Caption = 'a'
    Color = clBackground
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object lbl3: TLabel
    Left = 110
    Top = 21
    Width = 67
    Height = 13
    Caption = 'M'#234's/Ano Final'
    Color = clBackground
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentColor = False
    ParentFont = False
    Transparent = True
  end
  object lbl4: TLabel
    Left = 17
    Top = 76
    Width = 43
    Height = 13
    Caption = 'Processo'
  end
  object lbl5: TLabel
    Left = 17
    Top = 127
    Width = 45
    Height = 13
    Caption = 'Indicador'
  end
  object dbgResumo: TDBGrid
    Left = 17
    Top = 190
    Width = 659
    Height = 174
    Hint = 'Selecione o registro para alterar'
    TabStop = False
    DataSource = dsResumo
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ParentShowHint = False
    ShowHint = True
    TabOrder = 7
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'nome_pro'
        Title.Caption = 'Processo'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'Tahoma'
        Title.Font.Style = [fsBold]
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'desc_ind'
        Title.Caption = 'Indicador'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'Tahoma'
        Title.Font.Style = [fsBold]
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'valo_vin'
        Title.Caption = 'Valor'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'Tahoma'
        Title.Font.Style = [fsBold]
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'metaconcatenada'
        Title.Caption = 'Meta'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'Tahoma'
        Title.Font.Style = [fsBold]
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'mesano'
        Title.Caption = 'M'#234's/Ano'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'media'
        Title.Caption = 'M'#233'dia do Periodo'
        Visible = True
      end>
  end
  object pnl1: TPanel
    Left = 0
    Top = 182
    Width = 426
    Height = 72
    Align = alBottom
    BevelOuter = bvLowered
    ParentBackground = False
    TabOrder = 6
    object btnSair: TBitBtn
      Left = 352
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para sair'
      Align = alRight
      Caption = 'Sair'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = btnSairClick
    end
    object btnVideo: TBitBtn
      Left = 1
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para imprimir'
      Align = alLeft
      Caption = '&V'#237'deo'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = btnVideoClick
    end
    object btnImpressora: TBitBtn
      Left = 74
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para imprimir'
      Align = alLeft
      Caption = '&Impressora'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = btnImpressoraClick
    end
    object btnExcel: TBitBtn
      Left = 147
      Top = 1
      Width = 95
      Height = 70
      Hint = 'Clique para exportar as informa'#231#245'es para o Excel'
      Align = alLeft
      Caption = '&Exportar Excel'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        EFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = btnExcelClick
    end
  end
  object edtMesIni: TMaskEdit
    Left = 17
    Top = 40
    Width = 75
    Height = 26
    EditMask = '!99/0000;1;_'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    MaxLength = 7
    ParentFont = False
    TabOrder = 0
    Text = '  /    '
  end
  object edtMesFim: TMaskEdit
    Left = 110
    Top = 40
    Width = 75
    Height = 26
    EditMask = '!99/0000;1;_'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    MaxLength = 7
    ParentFont = False
    TabOrder = 1
    Text = '  /    '
  end
  object dblProcessos: TDBLookupComboBox
    Left = 17
    Top = 92
    Width = 329
    Height = 26
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'codi_pro'
    ListField = 'nome_pro'
    ListSource = dsProcessos
    ParentFont = False
    TabOrder = 2
  end
  object chkTodosProcessos: TCheckBox
    Left = 350
    Top = 96
    Width = 61
    Height = 17
    Caption = 'TODOS'
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    State = cbChecked
    TabOrder = 3
    OnClick = chkTodosProcessosClick
  end
  object dblIndicador: TDBLookupComboBox
    Left = 17
    Top = 143
    Width = 329
    Height = 26
    Enabled = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'Tahoma'
    Font.Style = []
    KeyField = 'codi_ind'
    ListField = 'desc_ind'
    ListSource = dsIndicadores
    ParentFont = False
    TabOrder = 4
  end
  object chkTodosInd: TCheckBox
    Left = 350
    Top = 147
    Width = 61
    Height = 17
    Caption = 'TODOS'
    Checked = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
    State = cbChecked
    TabOrder = 5
    OnClick = chkTodosIndClick
  end
  object dspResumo: TDataSetProvider
    DataSet = zqryResumo
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 111
    Top = 128
  end
  object cdsResumo: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspResumo'
    Left = 151
    Top = 128
    object cdsResumonome_pro1: TWideStringField
      FieldName = 'nome_pro'
      Size = 45
    end
    object cdsResumodesc_ind1: TWideStringField
      FieldName = 'desc_ind'
      Size = 80
    end
    object cdsResumovalo_vin: TFloatField
      FieldName = 'valo_vin'
    end
    object cdsResumometaconcatenada: TWideMemoField
      FieldName = 'metaconcatenada'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object cdsResumomesano: TWideMemoField
      FieldName = 'mesano'
      ReadOnly = True
      BlobType = ftWideMemo
    end
    object cdsResumomedia: TFloatField
      FieldName = 'media'
      ReadOnly = True
    end
  end
  object frxdbResumoIndicador: TfrxDBDataset
    UserName = 'frxdbResumoIndicador'
    CloseDataSource = False
    FieldAliases.Strings = (
      'extMes=extMes'
      'ano=ano'
      'periodo=periodo'
      'Valor=Valor'
      'ValorGrafico=ValorGrafico'
      'nome_pro=nome_pro'
      'desc_ind=desc_ind'
      'codi_ind=codi_ind'
      'Codigo=Codigo'
      'codi_vin=codi_vin'
      'obse_vin=obse_vin'
      'peri_vin=peri_vin'
      'proc_vin=proc_vin'
      'valo_vin=valo_vin'
      'sequ_vin=sequ_vin'
      'MetaConcatenada=MetaConcatenada'
      'Media=Media'
      'MesAno=MesAno')
    DataSet = cdsResumo
    BCDToCurrency = False
    Left = 128
    Top = 83
  end
  object zqryResumo: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT P.nome_pro, I.desc_ind, valo_vin, TC.valo_com ||  '#39' '#39' || ' +
        'I.meta_ind as MetaConcatenada, '
      
        '(SELECT AVG(valo_vin)    FROM valor_indicador    WHERE peri_vin ' +
        'between '#39'201601'#39' AND '#39'201612'#39'    AND codi_ind = V.codi_ind    '
      
        'GROUP BY V.codi_ind) as Media, SUBSTR(cast(peri_vin as varchar),' +
        '5,2) || '#39'/'#39' || SUBSTR(cast(peri_vin as varchar),1,4) AS MesAno  '
      'FROM valor_indicador V '
      'INNER JOIN Processos P ON P.codi_pro = V.proc_vin  '
      'INNER JOIN Indicadores I ON I.codi_ind = V.codi_ind  '
      
        'INNER JOIN indicadores_metas IM ON IM.met_codindicador = V.codi_' +
        'ind AND CAST(IM.met_anomes as integer) = peri_vin '
      
        'INNER JOIN Tabela_Combos TC ON TC.codi_com = IM.met_tipo AND TC.' +
        'tipo_com = 17 WHERE peri_vin between '#39'201601'#39' AND '#39'201612'#39' '
      'ORDER BY nome_pro, V.codi_ind, sequ_vin')
    Params = <>
    Left = 68
    Top = 128
  end
  object frxReport1: TfrxReport
    Version = '4.10.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40548.430550960600000000
    ReportOptions.LastChange = 42506.378577245370000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      'begin'
      ''
      'end.')
    Left = 60
    Top = 80
    Datasets = <
      item
        DataSet = frxdbResumoIndicador
        DataSetName = 'frxdbResumoIndicador'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object Detalhe: TfrxMasterData
        Height = 18.897650000000000000
        Top = 204.094620000000000000
        Width = 1046.929810000000000000
        DataSet = frxdbResumoIndicador
        DataSetName = 'frxdbResumoIndicador'
        RowCount = 0
        object frxControleDATA: TfrxMemoView
          Left = 0.377952760000000000
          Width = 604.346788660000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'desc_ind'
          DataSet = frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxdbResumoIndicador."desc_ind"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxControleDESCRICAO: TfrxMemoView
          Left = 661.795219450000000000
          Width = 185.196850393701000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'MetaConcatenada'
          DataSet = frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbResumoIndicador."MetaConcatenada"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxControleLANCAMENTO: TfrxMemoView
          Left = 846.992125980000000000
          Width = 93.732283460000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'valo_vin'
          DataSet = frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.6n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxdbResumoIndicador."valo_vin"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          Left = 941.102362200000000000
          Width = 106.582677165354000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'Media'
          DataSet = frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = '%2.6n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxdbResumoIndicador."Media"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          Left = 605.637795280000000000
          Width = 56.692886540000000000
          Height = 18.897650000000000000
          ShowHint = False
          DataField = 'MesAno'
          DataSet = frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbResumoIndicador."MesAno"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 52.913420000000000000
        Top = 283.464750000000000000
        Width = 1046.929810000000000000
        object LogoDestraManager: TfrxPictureView
          Left = 997.575450000000000000
          Top = 8.779530000000022000
          Width = 34.015770000000000000
          Height = 34.015770000000000000
          ShowHint = False
          Picture.Data = {
            
            D9}
          Stretched = False
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo11: TfrxMemoView
          Left = 3.779530000000000000
          Top = 30.236240000000010000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'P'#225'gina: [Page#] de [TotalPages#]')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = 3.779530000000000000
          Top = 26.456709999999990000
          Width = 982.677800000000300000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
      end
      object GroupHeader1: TfrxGroupHeader
        Height = 41.763772200000000000
        Top = 139.842610000000000000
        Width = 1046.929810000000000000
        Condition = 'frxdbResumoIndicador."nome_pro"'
        object Memo5: TfrxMemoView
          Top = 4.000000000000000000
          Width = 1046.929346220000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = cl3DLight
          DataSet = frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxdbResumoIndicador."nome_pro"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          Left = 661.637944170000000000
          Top = 22.677179999999990000
          Width = 185.196970000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Meta')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo8: TfrxMemoView
          Left = 846.834870240000000000
          Top = 22.677179999999990000
          Width = 94.110236220472400000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Resultado M'#234's')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo3: TfrxMemoView
          Left = 940.945270000000000000
          Top = 22.866122200000010000
          Width = 106.204724410000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'M'#233'dia Per'#237'odo')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo9: TfrxMemoView
          Left = 605.480520000000000000
          Top = 22.866122200000010000
          Width = 56.692950000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'M'#234's')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo1: TfrxMemoView
          Top = 22.677179999999990000
          Width = 604.724800000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Indicador')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object PageHeader1: TfrxPageHeader
        Height = 58.614112200000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        object Memo4: TfrxMemoView
          Top = 11.338590000000000000
          Width = 1046.929810000000000000
          Height = 34.015770000000000000
          ShowHint = False
          AutoWidth = True
          DataSetName = 'cdsParametroRelatorio'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -21
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'RELAT'#211'RIO RESUMO DE INDICADORES')
          ParentFont = False
        end
      end
    end
  end
  object zqryProcessos: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_pro, nome_pro '
      'FROM processos'
      'ORDER BY nome_pro')
    Params = <>
    Left = 260
    Top = 8
  end
  object dspProcessos: TDataSetProvider
    DataSet = zqryProcessos
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 300
    Top = 8
  end
  object cdsProcessos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspProcessos'
    Left = 340
    Top = 8
  end
  object dsProcessos: TDataSource
    DataSet = cdsProcessos
    Left = 380
    Top = 8
  end
  object cdsIndicadores: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspIndicadores'
    Left = 340
    Top = 56
  end
  object dsIndicadores: TDataSource
    DataSet = cdsIndicadores
    Left = 380
    Top = 56
  end
  object dspIndicadores: TDataSetProvider
    DataSet = zqryIndicadores
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 300
    Top = 56
  end
  object zqryIndicadores: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_ind, desc_ind '
      'FROM indicadores'
      'ORDER BY desc_ind')
    Params = <>
    Left = 260
    Top = 56
  end
  object dsResumo: TDataSource
    DataSet = cdsResumo
    Left = 196
    Top = 128
  end
end
