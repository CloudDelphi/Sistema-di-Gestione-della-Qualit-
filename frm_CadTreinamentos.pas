   ...
   if (Acesso(cUsuario, 3, 'exclusao') = 1) then begin
      if Application.MessageBox('Confirma a exclusão do registro ?', 'Confirmação', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES then begin
         ...
         with cdsGravar do begin
            Active:= False;
            CommandText:= ' DELETE FROM treinamentos' +
                          ' WHERE codi_tre = ' + QuotedStr(edtCodigo.Text);
            Execute;
         end;
   ...
   try
      with cdsGravar do begin
         ...
         if cOperacao = 'I' then begin
            CommandText:= ' INSERT INTO treinamentos (' +
                          ' codi_tre, desc_tre, tre_conteudo, tre_status)' +
                          ' VALUES(' +
                          QuotedStr(edtCodigo.Text) + ',' +
                          QuotedStr(edtTreinamento.Text) + ',' +
                          QuotedStr(mmoConteudo.Text) + ',' +
                          IntToStr(dblStatus.KeyValue) +
                          ');';
         end
         else begin
            CommandText:= ' UPDATE treinamentos SET' +
                          ' desc_tre = ' + QuotedStr(edtTreinamento.Text) + ',' +
                          ' tre_conteudo = ' + QuotedStr(mmoConteudo.Text) + ',' +
                          ' tre_status = ' + IntToStr(dblStatus.KeyValue) +
                          ' WHERE codi_tre = ' + QuotedStr(edtCodigo.Text);
         end;
         Execute;
      end;
...
begin
   case cbbCampo.ItemIndex of
      0: cdsTreinamentos.Locate('desc_tre',edtValor.Text, [loPartialKey]);
   end;
end;
...
procedure TFormCadTreinamentos.mmoConteudoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
   ...
   with cdsImprimir do begin
      ...
      CommandText:= ' SELECT codi_tre, desc_tre, tre_conteudo, tre_status, ' +
                    ' TC.valo_com as Status' +
                    ' FROM treinamentos' +
                    ' INNER JOIN tabela_combos TC ON TC.tipo_com = 15 AND TC.codi_com = tre_status' +
   ...
   with frxReport1 do begin
      if chkConteudo.Checked then begin
