object FormCadRNCFecha: TFormCadRNCFecha
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Fechamento de RNC'
  ClientHeight = 568
  ClientWidth = 751
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesktopCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pctFechaRNC: TJvgPageControl
    Left = 0
    Top = 0
    Width = 751
    Height = 496
    ActivePage = tsCadastro
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    TabStop = False
    OnChange = pctFechaRNCChange
    TabStyle.Borders = [fsdLeft, fsdTop, fsdRight, fsdBottom]
    TabStyle.BevelInner = bvNone
    TabStyle.BevelOuter = bvNone
    TabStyle.Bold = False
    TabStyle.BackgrColor = clBtnShadow
    TabStyle.Font.Charset = DEFAULT_CHARSET
    TabStyle.Font.Color = clBtnHighlight
    TabStyle.Font.Height = -11
    TabStyle.Font.Name = 'Arial'
    TabStyle.Font.Style = []
    TabStyle.CaptionHAlign = fhaCenter
    TabStyle.Gradient.Active = False
    TabStyle.Gradient.Orientation = fgdHorizontal
    TabSelectedStyle.Borders = [fsdLeft, fsdTop, fsdRight, fsdBottom]
    TabSelectedStyle.BevelInner = bvNone
    TabSelectedStyle.BevelOuter = bvNone
    TabSelectedStyle.Bold = False
    TabSelectedStyle.BackgrColor = clBtnFace
    TabSelectedStyle.Font.Charset = DEFAULT_CHARSET
    TabSelectedStyle.Font.Color = clBtnText
    TabSelectedStyle.Font.Height = -11
    TabSelectedStyle.Font.Name = 'Arial'
    TabSelectedStyle.Font.Style = []
    TabSelectedStyle.CaptionHAlign = fhaCenter
    TabSelectedStyle.Gradient.Active = False
    TabSelectedStyle.Gradient.Orientation = fgdHorizontal
    Options = [ftoAutoFontDirection, ftoExcludeGlyphs]
    object tsCadastro: TTabSheet
      Caption = 'Cadastro RNC'
      object lbl14: TLabel
        Left = 7
        Top = 151
        Width = 110
        Height = 13
        Caption = 'N'#186' Ordem de Produ'#231#227'o'
      end
      object lbl15: TLabel
        Left = 125
        Top = 151
        Width = 28
        Height = 13
        Caption = 'Custo'
        Color = clBackground
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = True
      end
      object lbl4: TLabel
        Left = 7
        Top = 195
        Width = 89
        Height = 13
        Caption = 'N'#227'o Conformidade'
      end
      object lbl2: TLabel
        Left = 107
        Top = 12
        Width = 23
        Height = 13
        Caption = 'Data'
      end
      object lbl6: TLabel
        Left = 514
        Top = 12
        Width = 71
        Height = 13
        Caption = 'Motivo do RNC'
      end
      object lbl5: TLabel
        Left = 7
        Top = 12
        Width = 62
        Height = 13
        Caption = 'Identifica'#231#227'o'
      end
      object lbl1: TLabel
        Left = 217
        Top = 12
        Width = 53
        Height = 13
        Caption = 'Emitido por'
      end
      object lbl16: TLabel
        Left = 7
        Top = 60
        Width = 43
        Height = 13
        Caption = 'Processo'
      end
      object lbl3: TLabel
        Left = 81
        Top = 243
        Width = 61
        Height = 13
        Caption = 'Respons'#225'vel'
      end
      object lbl8: TLabel
        Left = 7
        Top = 243
        Width = 47
        Height = 13
        Caption = 'Procede ?'
      end
      object lbl7: TLabel
        Left = 304
        Top = 60
        Width = 93
        Height = 13
        Caption = 'Requisito da Norma'
      end
      object lbl11: TLabel
        Left = 559
        Top = 61
        Width = 34
        Height = 13
        Caption = 'Origem'
      end
      object lbl12: TLabel
        Left = 7
        Top = 293
        Width = 50
        Height = 13
        Caption = 'Disposi'#231#227'o'
      end
      object lbl17: TLabel
        Left = 586
        Top = 424
        Width = 31
        Height = 13
        Caption = 'Status'
      end
      object lbl23: TLabel
        Left = 7
        Top = 107
        Width = 33
        Height = 13
        Caption = 'Cliente'
        Color = clBackground
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = True
      end
      object lbl24: TLabel
        Left = 377
        Top = 107
        Width = 55
        Height = 13
        Caption = 'Fornecedor'
        Color = clBackground
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = True
      end
      object sbVisualizarNC: TSpeedButton
        Left = 714
        Top = 217
        Width = 25
        Height = 26
        Hint = 'Clique para exibir/ocultar o texto da N'#227'o Conformidade'
        Caption = '...'
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        OnClick = sbVisualizarNCClick
      end
      object sbVisualizarDisp: TSpeedButton
        Left = 549
        Top = 439
        Width = 25
        Height = 26
        Hint = 'Clique para exibir/ocultar o texto da Disposi'#231#227'o'
        Caption = '...'
        NumGlyphs = 2
        ParentShowHint = False
        ShowHint = True
        OnClick = sbVisualizarDispClick
      end
      object lbl9: TLabel
        Left = 587
        Top = 293
        Width = 59
        Height = 13
        Caption = 'Fechamento'
      end
      object edtCusto: TCurrencyEdit
        Tag = 30
        Left = 125
        Top = 166
        Width = 122
        Height = 26
        Hint = 'Custo'
        Margins.Left = 5
        Margins.Top = 1
        AutoSize = False
        DisplayFormat = ',R$ 0.00;-,R$ 0.00'
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 19
      end
      object edtOrdemProd: TEdit
        Tag = 30
        Left = 7
        Top = 166
        Width = 110
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 10
        ParentFont = False
        TabOrder = 18
      end
      object dblTipo: TDBLookupComboBox
        Tag = 30
        Left = 514
        Top = 27
        Width = 225
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_com'
        ListField = 'valo_com'
        ListSource = dsMotivo
        ParentFont = False
        TabOrder = 3
      end
      object edtIdentificacao: TEdit
        Tag = 30
        Left = 7
        Top = 27
        Width = 95
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object dblProcesso: TDBLookupComboBox
        Tag = 30
        Left = 7
        Top = 76
        Width = 291
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_pro'
        ListField = 'nome_pro'
        ListSource = dsProcessos
        ParentFont = False
        TabOrder = 4
      end
      object dblResponsavel: TDBLookupComboBox
        Left = 81
        Top = 260
        Width = 464
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_col'
        ListField = 'nome_col'
        ListSource = dsResponsavel
        ParentFont = False
        TabOrder = 9
      end
      object dblProcede: TDBLookupComboBox
        Left = 7
        Top = 259
        Width = 68
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_com'
        ListField = 'valo_com'
        ListSource = dsProcede
        ParentFont = False
        TabOrder = 8
      end
      object dtData: TDateEdit
        Tag = 30
        Left = 107
        Top = 27
        Width = 105
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 1
      end
      object dblEmitido: TDBLookupComboBox
        Tag = 30
        Left = 217
        Top = 27
        Width = 291
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_col'
        ListField = 'nome_col'
        ListSource = dsEmitido
        ParentFont = False
        TabOrder = 2
      end
      object edtReqNorma: TEdit
        Tag = 30
        Left = 304
        Top = 76
        Width = 249
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
      end
      object dblOrigem: TDBLookupComboBox
        Tag = 30
        Left = 559
        Top = 76
        Width = 180
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_com'
        ListField = 'valo_com'
        ListSource = dsOrigem
        ParentFont = False
        TabOrder = 6
      end
      object mmoNaoConformidade: TMemo
        Tag = 30
        Left = 7
        Top = 211
        Width = 701
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Lines.Strings = (
          '')
        ParentFont = False
        TabOrder = 7
        OnKeyPress = mmoNaoConformidadeKeyPress
      end
      object mmoDisposicao: TMemo
        Left = 7
        Top = 309
        Width = 536
        Height = 157
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        Lines.Strings = (
          '')
        ParentFont = False
        ScrollBars = ssVertical
        TabOrder = 10
        OnKeyPress = mmoNaoConformidadeKeyPress
      end
      object dblStatus: TDBLookupComboBox
        Left = 586
        Top = 440
        Width = 153
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_com'
        ListField = 'valo_com'
        ListSource = dsStatus
        ParentFont = False
        TabOrder = 11
      end
      object dblClientes: TDBLookupComboBox
        Tag = 30
        Left = 7
        Top = 122
        Width = 362
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'cli_codigo'
        ListField = 'cli_nome'
        ListSource = dsClientes
        ParentFont = False
        TabOrder = 12
      end
      object dblForn: TDBLookupComboBox
        Tag = 30
        Left = 377
        Top = 122
        Width = 362
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'forn_codigo'
        ListField = 'forn_nome'
        ListSource = dsForn
        ParentFont = False
        TabOrder = 13
      end
      object pnlEmail: TPanel
        Left = 580
        Top = 293
        Width = 578
        Height = 188
        TabOrder = 15
        object lbl29: TLabel
          Left = 16
          Top = 61
          Width = 28
          Height = 13
          Caption = 'E-mail'
        end
        object lbl30: TLabel
          Left = 16
          Top = 41
          Width = 66
          Height = 13
          Caption = 'Colaborador: '
        end
        object lblNomeCol: TLabel
          Left = 88
          Top = 41
          Width = 27
          Height = 13
          Caption = 'Nome'
        end
        object pnl11: TPanel
          Left = 1
          Top = 115
          Width = 576
          Height = 72
          Align = alBottom
          TabOrder = 1
          object btnSairEmail: TBitBtn
            Left = 502
            Top = 1
            Width = 73
            Height = 70
            Hint = 'Clique para sair'
            Align = alRight
            Caption = 'Sair'
            DoubleBuffered = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentDoubleBuffered = False
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 0
            OnClick = btnSairEmailClick
          end
          object btnGravarEmail: TBitBtn
            Left = 1
            Top = 1
            Width = 73
            Height = 70
            Hint = 'Clique para gravar o e-mail do colaborador'
            Align = alLeft
            Caption = '&Gravar'
            DoubleBuffered = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentDoubleBuffered = False
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 1
            OnClick = btnGravarEmailClick
          end
        end
        object pnl13: TPanel
          Left = 1
          Top = 1
          Width = 576
          Height = 29
          Align = alTop
          Alignment = taLeftJustify
          Caption = ' Cadastro de E-mail'
          Color = clGradientActiveCaption
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object edtEmail: TEdit
          Left = 16
          Top = 78
          Width = 531
          Height = 26
          CharCase = ecLowerCase
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
      end
      object btnAceitar: TBitBtn
        Left = 587
        Top = 309
        Width = 73
        Height = 70
        Hint = 'Clique para aceitar a disposi'#231#227'o'
        Caption = 'Aceitar'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 16
        OnClick = btnAceitarClick
      end
      object btnRecusar: TBitBtn
        Left = 666
        Top = 309
        Width = 73
        Height = 70
        Hint = 'Clique para recusar a disposi'#231#227'o'
        Caption = 'Recusar'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 17
        OnClick = btnRecusarClick
      end
      object pnlTexto: TPanel
        Left = 689
        Top = 209
        Width = 730
        Height = 270
        TabOrder = 14
        object lblTituloTexto: TLabel
          Left = 4
          Top = 1
          Width = 26
          Height = 13
          Caption = 'T'#237'tulo'
        end
        object pnl12: TPanel
          Left = 1
          Top = 197
          Width = 728
          Height = 72
          Align = alBottom
          TabOrder = 0
          object btnFechar: TBitBtn
            Left = 654
            Top = 1
            Width = 73
            Height = 70
            Hint = 'Clique para fechar'
            Align = alRight
            Caption = 'Fechar'
            DoubleBuffered = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            Glyph.Data = {
              
              FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
            Layout = blGlyphTop
            NumGlyphs = 2
            ParentDoubleBuffered = False
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            TabOrder = 0
            OnClick = btnFecharClick
          end
        end
        object mmoTexto: TMemo
          Left = 4
          Top = 15
          Width = 723
          Height = 185
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          ParentShowHint = False
          ReadOnly = True
          ScrollBars = ssVertical
          ShowHint = True
          TabOrder = 1
          OnKeyPress = mmoDescricaoAcaoKeyPress
        end
      end
    end
    object tsHistorico: TTabSheet
      Caption = 'Hist'#243'rico'
      ImageIndex = 1
      object lbl10: TLabel
        Left = 107
        Top = 12
        Width = 23
        Height = 13
        Caption = 'Data'
      end
      object lbl13: TLabel
        Left = 7
        Top = 12
        Width = 62
        Height = 13
        Caption = 'Identifica'#231#227'o'
      end
      object dbgRegistros: TDBGrid
        Left = 1
        Top = 59
        Width = 740
        Height = 314
        Hint = 'Duplo clique para visualizar detalhes'
        TabStop = False
        DataSource = dsHistorico
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'his_data'
            Title.Caption = 'Data'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'his_historico'
            Title.Caption = 'Hist'#243'rico'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 150
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'his_usuario'
            Title.Caption = 'Usu'#225'rio'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 150
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'his_disposicao'
            Title.Caption = 'Disposi'#231#227'o'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 320
            Visible = True
          end>
      end
      object edtIdentificacaoHist: TEdit
        Left = 7
        Top = 27
        Width = 95
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
      object dtDataHist: TDateEdit
        Left = 107
        Top = 27
        Width = 105
        Height = 26
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 2
      end
    end
  end
  object pnl1: TPanel
    Left = 0
    Top = 496
    Width = 751
    Height = 72
    Align = alBottom
    TabOrder = 1
    object btnGravar: TBitBtn
      Left = 74
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para gravar o registro'
      Align = alLeft
      Caption = '&Gravar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = btnGravarClick
    end
    object btnCancelar: TBitBtn
      Left = 147
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para cancelar a opera'#231#227'o'
      Align = alLeft
      Caption = '&Cancelar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = btnCancelarClick
    end
    object btnImprimir: TBitBtn
      Left = 220
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para imprimir'
      Align = alLeft
      Caption = '&Imprimir'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = btnImprimirClick
    end
    object btnSair: TBitBtn
      Left = 677
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para sair'
      Align = alRight
      Caption = 'Sair'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnClick = btnSairClick
    end
    object btnAlterar: TBitBtn
      Left = 1
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para alterar um registro'
      Align = alLeft
      Caption = '&Alterar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFDFDFDAEACAB757372817E7ED6D5D5FFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = btnAlterarClick
    end
    object btnPDCA: TBitBtn
      Left = 293
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para abrir o cadastro de PDCA'
      Align = alLeft
      Caption = '&PDCA'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      Visible = False
      OnClick = btnPDCAClick
    end
  end
  object pnlImprimir: TPanel
    Left = 703
    Top = 351
    Width = 365
    Height = 188
    TabOrder = 2
    object pnl2: TPanel
      Left = 1
      Top = 115
      Width = 363
      Height = 72
      Align = alBottom
      TabOrder = 1
      object btnSairImp: TBitBtn
        Left = 289
        Top = 1
        Width = 73
        Height = 70
        Hint = 'Clique para sair'
        Align = alRight
        Caption = 'Sair'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnSairImpClick
      end
      object btnVideo: TBitBtn
        Left = 1
        Top = 1
        Width = 73
        Height = 70
        Hint = 'Clique para imprimir'
        Align = alLeft
        Caption = '&V'#237'deo'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = btnVideoClick
      end
      object btnImpressora: TBitBtn
        Left = 74
        Top = 1
        Width = 73
        Height = 70
        Hint = 'Clique para imprimir'
        Align = alLeft
        Caption = '&Impressora'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        OnClick = btnImpressoraClick
      end
    end
    object pnl3: TPanel
      Left = 1
      Top = 1
      Width = 363
      Height = 29
      Align = alTop
      Alignment = taLeftJustify
      Caption = ' Impress'#227'o'
      Color = clGradientActiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
  end
  object zqryGravar: TZQuery
    Connection = dm.Conexao
    Params = <>
    Left = 488
    Top = 104
  end
  object dspGravar: TDataSetProvider
    DataSet = zqryGravar
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 528
    Top = 104
  end
  object cdsGravar: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspGravar'
    Left = 568
    Top = 104
  end
  object zqryImprimir: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT R.rnc_codigo, rnc_identificacao, rnc_data, rnc_emitido, C' +
        '.nome_col as Emitido, rnc_motivo, '
      
        'TC1.valo_com as Motivo, rnc_processo, P.nome_pro as Processo, rn' +
        'c_setor, rnc_origem, TC.valo_com as Origem, '
      
        'rnc_cliente, CL.cli_nome as Cliente, rnc_fornecedor, F.forn_nome' +
        ' as Fornecedor, rnc_consumidor, rnc_nconformidade, '
      
        'rnc_procede, TC3.valo_com as Procede, rnc_responsavel, C1.nome_c' +
        'ol as Responsavel, rnc_departamento, '
      
        'rnc_relacionamento, rnc_representante, rnc_status, TC2.valo_com ' +
        'as Status, rnc_disposicao, H.*'
      'FROM rnc R'
      'LEFT JOIN rnc_historico H ON H.rnc_codigo = R.rnc_codigo'
      'INNER JOIN colaboradores C ON C.codi_col = rnc_emitido '
      'INNER JOIN colaboradores C1 ON C1.codi_col = rnc_responsavel '
      'INNER JOIN processos P ON P.codi_pro = rnc_processo'
      
        'INNER JOIN tabela_combos TC ON TC.tipo_com = 33 AND TC.codi_com ' +
        '= rnc_origem'
      
        'INNER JOIN tabela_combos TC1 ON TC1.tipo_com = 32 AND TC1.codi_c' +
        'om = rnc_motivo'
      
        'INNER JOIN tabela_combos TC2 ON TC2.tipo_com = 35 AND TC2.codi_c' +
        'om = rnc_status'
      
        'INNER JOIN tabela_combos TC3 ON TC3.tipo_com = 14 AND TC3.codi_c' +
        'om = rnc_procede'
      'INNER JOIN fornecedores F ON F.forn_codigo = rnc_fornecedor'
      'INNER JOIN clientes CL ON CL.cli_codigo = rnc_cliente')
    Params = <>
    Left = 488
    Top = 152
  end
  object dspImprimir: TDataSetProvider
    DataSet = zqryImprimir
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 528
    Top = 152
  end
  object cdsImprimir: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspImprimir'
    Left = 568
    Top = 152
    object cdsImprimirrnc_codigo: TIntegerField
      FieldName = 'rnc_codigo'
      Required = True
    end
    object cdsImprimirrnc_identificacao: TWideStringField
      FieldName = 'rnc_identificacao'
      Required = True
      Size = 7
    end
    object cdsImprimirrnc_data: TDateTimeField
      FieldName = 'rnc_data'
    end
    object cdsImprimirrnc_emitido: TIntegerField
      FieldName = 'rnc_emitido'
    end
    object cdsImprimiremitido: TWideStringField
      FieldName = 'emitido'
      ReadOnly = True
      Size = 45
    end
    object cdsImprimirrnc_motivo: TIntegerField
      FieldName = 'rnc_motivo'
    end
    object cdsImprimirmotivo: TWideStringField
      FieldName = 'motivo'
      ReadOnly = True
      Size = 50
    end
    object cdsImprimirrnc_processo: TIntegerField
      FieldName = 'rnc_processo'
    end
    object cdsImprimirprocesso: TWideStringField
      FieldName = 'processo'
      ReadOnly = True
      Size = 45
    end
    object cdsImprimirrnc_setor: TWideStringField
      FieldName = 'rnc_setor'
      Size = 25
    end
    object cdsImprimirrnc_origem: TIntegerField
      FieldName = 'rnc_origem'
    end
    object cdsImprimirorigem: TWideStringField
      FieldName = 'origem'
      ReadOnly = True
      Size = 50
    end
    object cdsImprimirrnc_cliente: TIntegerField
      FieldName = 'rnc_cliente'
    end
    object cdsImprimirrnc_fornecedor: TIntegerField
      FieldName = 'rnc_fornecedor'
    end
    object cdsImprimirrnc_consumidor: TWideStringField
      FieldName = 'rnc_consumidor'
      Size = 60
    end
    object cdsImprimirrnc_nconformidade: TWideMemoField
      FieldName = 'rnc_nconformidade'
      BlobType = ftWideMemo
    end
    object cdsImprimirrnc_procede: TIntegerField
      FieldName = 'rnc_procede'
    end
    object cdsImprimirrnc_responsavel: TIntegerField
      FieldName = 'rnc_responsavel'
    end
    object cdsImprimirresponsavel: TWideStringField
      FieldName = 'responsavel'
      ReadOnly = True
      Size = 45
    end
    object cdsImprimirrnc_departamento: TWideStringField
      FieldName = 'rnc_departamento'
      Size = 25
    end
    object cdsImprimirrnc_relacionamento: TIntegerField
      FieldName = 'rnc_relacionamento'
    end
    object cdsImprimirrnc_representante: TWideStringField
      FieldName = 'rnc_representante'
      Size = 25
    end
    object cdsImprimirrnc_status: TIntegerField
      FieldName = 'rnc_status'
    end
    object cdsImprimirstatus: TWideStringField
      FieldName = 'status'
      ReadOnly = True
      Size = 50
    end
    object cdsImprimirrnc_disposicao: TWideMemoField
      FieldName = 'rnc_disposicao'
      BlobType = ftWideMemo
    end
    object cdsImprimirhis_codigo: TIntegerField
      FieldName = 'his_codigo'
      Required = True
    end
    object cdsImprimirrnc_codigo_1: TIntegerField
      FieldName = 'rnc_codigo_1'
      Required = True
    end
    object cdsImprimirhis_data: TDateTimeField
      FieldName = 'his_data'
    end
    object cdsImprimirhis_historico: TWideStringField
      FieldName = 'his_historico'
      Size = 250
    end
    object cdsImprimirhis_usuario: TWideStringField
      FieldName = 'his_usuario'
      Size = 30
    end
    object cdsImprimirhis_disposicao: TWideMemoField
      FieldName = 'his_disposicao'
      BlobType = ftWideMemo
    end
    object cdsImprimircliente: TWideStringField
      FieldName = 'cliente'
      ReadOnly = True
      Size = 200
    end
    object cdsImprimirfornecedor: TWideStringField
      FieldName = 'fornecedor'
      ReadOnly = True
      Size = 80
    end
    object cdsImprimirprocede: TWideStringField
      FieldName = 'procede'
      ReadOnly = True
      Size = 50
    end
  end
  object zqryRNC: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT rnc_codigo, rnc_identificacao, rnc_data, rnc_emitido, rnc' +
        '_motivo,'
      'rnc_processo, rnc_setor, rnc_origem, rnc_cliente, '
      'rnc_fornecedor, rnc_consumidor, rnc_nconformidade, '
      
        'rnc_procede, rnc_responsavel, rnc_departamento, rnc_relacionamen' +
        'to, '
      'rnc_representante, rnc_status, rnc_disposicao'
      'FROM rnc R')
    Params = <>
    Left = 288
    Top = 184
  end
  object dspRNC: TDataSetProvider
    DataSet = zqryRNC
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 328
    Top = 184
  end
  object cdsRNC: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspRNC'
    Left = 368
    Top = 184
  end
  object dsRNC: TDataSource
    DataSet = cdsRNC
    Left = 408
    Top = 184
  end
  object cdsMotivo: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspMotivo'
    Left = 448
    Top = 376
    object lrgntfldMotivocodi_com: TLargeintField
      FieldName = 'codi_com'
      Required = True
    end
    object cdsMotivovalo_com: TWideStringField
      FieldName = 'valo_com'
      Size = 50
    end
  end
  object dsMotivo: TDataSource
    DataSet = cdsMotivo
    Left = 488
    Top = 376
  end
  object dspMotivo: TDataSetProvider
    DataSet = zqryMotivo
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 408
    Top = 376
  end
  object zqryMotivo: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_com, valo_com FROM tabela_combos')
    Params = <>
    Left = 368
    Top = 376
  end
  object opd1: TOpenDialog
    Filter = 'Todos os arquivos|*.*'
    Title = 'Selecionar Arquivo'
    Left = 608
    Top = 368
  end
  object zqryProcessos: TZQuery
    Connection = dm.Conexao
    Params = <>
    Left = 368
    Top = 280
  end
  object dspProcessos: TDataSetProvider
    DataSet = zqryProcessos
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 408
    Top = 280
  end
  object cdsProcessos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspProcessos'
    Left = 448
    Top = 280
  end
  object dsProcessos: TDataSource
    DataSet = cdsProcessos
    Left = 488
    Top = 280
  end
  object zqryEmitido: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_col, nome_col, col_email'
      'FROM Colaboradores'
      'ORDER BY nome_col'
      '')
    Params = <>
    Left = 128
    Top = 264
  end
  object dspEmitido: TDataSetProvider
    DataSet = zqryEmitido
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 168
    Top = 264
  end
  object cdsEmitido: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspEmitido'
    Left = 208
    Top = 264
    object cdsEmitidocodi_col: TLargeintField
      FieldName = 'codi_col'
      Required = True
    end
    object cdsEmitidonome_col: TWideStringField
      FieldName = 'nome_col'
      Size = 45
    end
    object cdsEmitidocol_email: TWideStringField
      FieldName = 'col_email'
      Size = 80
    end
  end
  object dsEmitido: TDataSource
    DataSet = cdsEmitido
    Left = 248
    Top = 264
  end
  object zqryProcede: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_com, valo_com FROM tabela_combos')
    Params = <>
    Left = 128
    Top = 312
  end
  object dspProcede: TDataSetProvider
    DataSet = zqryProcede
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 168
    Top = 312
  end
  object cdsProcede: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspProcede'
    Left = 208
    Top = 312
    object LargeintField2: TLargeintField
      FieldName = 'codi_com'
      Required = True
    end
    object WideStringField2: TWideStringField
      FieldName = 'valo_com'
      Size = 50
    end
  end
  object dsProcede: TDataSource
    DataSet = cdsProcede
    Left = 248
    Top = 312
  end
  object zqryOrigem: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_com, valo_com FROM tabela_combos')
    Params = <>
    Left = 368
    Top = 232
  end
  object dspOrigem: TDataSetProvider
    DataSet = zqryOrigem
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 408
    Top = 232
  end
  object cdsOrigem: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspOrigem'
    Left = 448
    Top = 232
    object LargeintField3: TLargeintField
      FieldName = 'codi_com'
      Required = True
    end
    object WideStringField3: TWideStringField
      FieldName = 'valo_com'
      Size = 50
    end
  end
  object dsOrigem: TDataSource
    DataSet = cdsOrigem
    Left = 488
    Top = 232
  end
  object zqryResponsavel: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_col, nome_col, col_email'
      'FROM Colaboradores'
      'ORDER BY nome_col')
    Params = <>
    Left = 368
    Top = 328
  end
  object dspResponsavel: TDataSetProvider
    DataSet = zqryResponsavel
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 408
    Top = 328
  end
  object cdsResponsavel: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspResponsavel'
    Left = 448
    Top = 328
    object LargeintField1: TLargeintField
      FieldName = 'codi_col'
      Required = True
    end
    object WideStringField1: TWideStringField
      FieldName = 'nome_col'
      Size = 45
    end
    object cdsResponsavelcol_email: TWideStringField
      FieldName = 'col_email'
      Size = 80
    end
  end
  object dsResponsavel: TDataSource
    DataSet = cdsResponsavel
    Left = 488
    Top = 328
  end
  object frxReport1: TfrxReport
    Version = '4.10.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40548.430550960600000000
    ReportOptions.LastChange = 42853.456385937500000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      'begin'
      ''
      'end.')
    Left = 280
    Top = 112
    Datasets = <
      item
        DataSet = frxDBRNCFecha
        DataSetName = 'frxDBRNCFecha'
      end>
    Variables = <
      item
        Name = ' New Category1'
        Value = Null
      end
      item
        Name = 'status'
        Value = Null
      end>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        Height = 580.338900000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
        object Memo4: TfrxMemoView
          Top = 22.267780000000000000
          Width = 718.110700000000000000
          Height = 34.015770000000000000
          ShowHint = False
          AutoWidth = True
          DataSetName = 'cdsParametroRelatorio'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -21
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'RNC - REGISTRO DE N'#195'O CONFORMIDADE')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Width = 1046.929810000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo11: TfrxMemoView
          Top = 71.252010000000000000
          Width = 718.110236220000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = cl3DLight
          DataSet = FormResumoInd.frxdbResumoIndicador
          DataSetName = 'frxdbResumoIndicador'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          Memo.UTF8W = (
            'Identifica'#231#227'o: [frxdbRNCFecha."rnc_identificacao"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxControleDESCRICAO: TfrxMemoView
          Left = 7.936954170000000000
          Top = 114.724490000000000000
          Width = 71.811006540000000000
          Height = 30.236220472440900000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."rnc_data"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo9: TfrxMemoView
          Left = 7.559060000000000000
          Top = 96.047310000000000000
          Width = 71.811070000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Data')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo5: TfrxMemoView
          Left = 82.763794170000000000
          Top = 114.724490000000000000
          Width = 302.362336540000000000
          Height = 30.236220470000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Emitido"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          Left = 82.385900000000000000
          Top = 96.047310000000000000
          Width = 302.362400000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Emitido por')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo13: TfrxMemoView
          Left = 387.567134170000000000
          Top = 114.724490000000000000
          Width = 166.299256540000000000
          Height = 30.236220470000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Motivo"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          Left = 387.189240000000000000
          Top = 96.047310000000000000
          Width = 166.299320000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Motivo')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo15: TfrxMemoView
          Left = 558.386210000000000000
          Top = 114.897637800000000000
          Width = 158.740260000000000000
          Height = 30.236227800000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Origem"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo16: TfrxMemoView
          Left = 557.031850000000000000
          Top = 96.047310000000000000
          Width = 153.070866141732000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Origem')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo3: TfrxMemoView
          Left = 7.936954170000000000
          Top = 250.637910000000000000
          Width = 702.992516540000000000
          Height = 120.944960000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haBlock
          Memo.UTF8W = (
            '[frxdbRNCFecha."rnc_nconformidade"]')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 8.559060000000000000
          Top = 231.960730000000000000
          Width = 702.992580000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Descri'#231#227'o da N'#227'o Conformidade')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo7: TfrxMemoView
          Left = 10.330894170000000000
          Top = 207.637910000000000000
          Width = 56.692886540000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Procede"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          Left = 9.953000000000000000
          Top = 188.960730000000000000
          Width = 56.692950000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Procede')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo17: TfrxMemoView
          Left = 70.385826770000000000
          Top = 207.858380000000000000
          Width = 302.362336540000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Responsavel"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo18: TfrxMemoView
          Left = 70.385826770000000000
          Top = 189.181200000000000000
          Width = 302.362400000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Respons'#225'vel')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo19: TfrxMemoView
          Left = 8.692913390000000000
          Top = 395.141930000000000000
          Width = 702.992125980000000000
          Height = 136.063080000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haBlock
          Memo.UTF8W = (
            '[frxdbRNCFecha."rnc_disposicao"]')
          ParentFont = False
        end
        object Memo20: TfrxMemoView
          Left = 8.692913390000000000
          Top = 376.464750000000000000
          Width = 702.992580000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Disposi'#231#227'o')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo35: TfrxMemoView
          Left = 7.559060000000000000
          Top = 166.299320000000000000
          Width = 347.716696540000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Cliente"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          Left = 7.559060000000000000
          Top = 147.622140000000000000
          Width = 347.716760000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Cliente')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo37: TfrxMemoView
          Left = 359.055350000000000000
          Top = 166.299320000000000000
          Width = 351.496290000000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Fornecedor"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo38: TfrxMemoView
          Left = 359.055350000000000000
          Top = 147.401670000000000000
          Width = 351.496290000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Fornecedor')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo39: TfrxMemoView
          Left = 375.968770000000000000
          Top = 207.653680000000000000
          Width = 336.378106540000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Color = clWhite
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxdbRNCFecha."Processo"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo40: TfrxMemoView
          Left = 375.968770000000000000
          Top = 188.976500000000000000
          Width = 334.488188976378000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Processo')
          ParentFont = False
          VAlign = vaBottom
        end
        object MFase: TfrxMemoView
          Left = 395.732530000000000000
          Top = 70.811070000000000000
          Width = 321.260050000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haRight
          Memo.UTF8W = (
            'Status: [frxdbRNCFecha."Status"]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Left = 10.338590000000000000
          Top = 561.559055118110000000
          Width = 90.708720000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Data')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo21: TfrxMemoView
          Left = 101.047310000000000000
          Top = 561.693260000000000000
          Width = 219.212740000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Hist'#243'rico')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo22: TfrxMemoView
          Left = 320.260050000000000000
          Top = 561.693260000000000000
          Width = 177.637910000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Usu'#225'rio')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo23: TfrxMemoView
          Left = 497.897960000000000000
          Top = 561.693260000000000000
          Width = 215.433070870000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Disposi'#231#227'o')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo24: TfrxMemoView
          Left = 10.338590000000000000
          Top = 542.472790000000000000
          Width = 702.992580000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Color = clScrollBar
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Hist'#243'rico')
          ParentFont = False
          VAlign = vaBottom
        end
      end
      object Detalhe: TfrxMasterData
        Height = 18.897650000000000000
        Top = 661.417750000000000000
        Width = 718.110700000000000000
        DataSet = frxDBRNCFecha
        DataSetName = 'frxDBRNCFecha'
        RowCount = 0
        object Memo2: TfrxMemoView
          Left = 10.204724410000000000
          Width = 90.708656540000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DataField = 'his_data'
          DataSet = frxDBRNCFecha
          DataSetName = 'frxDBRNCFecha'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBRNCFecha."his_data"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          Left = 100.913385830000000000
          Width = 219.212598430000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DataField = 'his_historico'
          DataSet = frxDBRNCFecha
          DataSetName = 'frxDBRNCFecha'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBRNCFecha."his_historico"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo26: TfrxMemoView
          Left = 320.125984251969000000
          Width = 177.637795280000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DataField = 'his_usuario'
          DataSet = frxDBRNCFecha
          DataSetName = 'frxDBRNCFecha'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBRNCFecha."his_usuario"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo27: TfrxMemoView
          Left = 497.763779530000000000
          Width = 215.433070870000000000
          Height = 18.897650000000000000
          ShowHint = False
          BrushStyle = bsClear
          DataField = 'his_disposicao'
          DataSet = frxDBRNCFecha
          DataSetName = 'frxDBRNCFecha'
          DisplayFormat.DecimalSeparator = ','
          DisplayFormat.FormatStr = 'dd/mm/yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBRNCFecha."his_disposicao"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 57.031540000000000000
        Top = 740.787880000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          Left = 925.945270000000000000
          Top = 3.779530000000022000
          Width = 117.165430000000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          HAlign = haRight
          Memo.UTF8W = (
            'P'#225'gina: [Page#]')
        end
        object Memo34: TfrxMemoView
          Left = 3.779530000000000000
          Top = 34.015770000000200000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'P'#225'gina: [Page#] de [TotalPages#]')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = 3.779530000000000000
          Top = 30.236239999999950000
          Width = 634.960649450000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
        object Picture2: TfrxPictureView
          Left = 663.622450000000000000
          Top = 3.779530000000022000
          Width = 37.795300000000000000
          Height = 37.795300000000000000
          ShowHint = False
          Picture.Data = {
            
            D9}
          Stretched = False
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
      end
    end
  end
  object frxDBRNCFecha: TfrxDBDataset
    UserName = 'frxDBRNCFecha'
    CloseDataSource = False
    FieldAliases.Strings = (
      'rnc_codigo=rnc_codigo'
      'rnc_identificacao=rnc_identificacao'
      'rnc_data=rnc_data'
      'rnc_emitido=rnc_emitido'
      'emitido=emitido'
      'rnc_motivo=rnc_motivo'
      'motivo=motivo'
      'rnc_processo=rnc_processo'
      'processo=processo'
      'rnc_setor=rnc_setor'
      'rnc_origem=rnc_origem'
      'origem=origem'
      'rnc_cliente=rnc_cliente'
      'rnc_fornecedor=rnc_fornecedor'
      'rnc_consumidor=rnc_consumidor'
      'rnc_nconformidade=rnc_nconformidade'
      'rnc_procede=rnc_procede'
      'rnc_responsavel=rnc_responsavel'
      'responsavel=responsavel'
      'rnc_departamento=rnc_departamento'
      'rnc_relacionamento=rnc_relacionamento'
      'rnc_representante=rnc_representante'
      'rnc_status=rnc_status'
      'status=status'
      'rnc_disposicao=rnc_disposicao'
      'his_codigo=his_codigo'
      'rnc_codigo_1=rnc_codigo_1'
      'his_data=his_data'
      'his_historico=his_historico'
      'his_usuario=his_usuario'
      'his_disposicao=his_disposicao'
      'cliente=cliente'
      'fornecedor=fornecedor'
      'procede=procede')
    DataSet = cdsImprimir
    BCDToCurrency = False
    Left = 368
    Top = 112
  end
  object zqryClientes: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT cli_codigo, cli_nome '
      'FROM clientes'
      'ORDER BY cli_nome')
    Params = <>
    Left = 128
    Top = 408
  end
  object dspClientes: TDataSetProvider
    DataSet = zqryClientes
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 168
    Top = 408
  end
  object cdsClientes: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspClientes'
    Left = 208
    Top = 408
    object cdsClientescli_codigo: TIntegerField
      FieldName = 'cli_codigo'
      Required = True
    end
    object cdsClientescli_nome: TWideStringField
      FieldName = 'cli_nome'
      Size = 200
    end
  end
  object dsClientes: TDataSource
    DataSet = cdsClientes
    Left = 248
    Top = 408
  end
  object zqryForn: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT forn_codigo, forn_nome '
      'FROM fornecedores'
      'ORDER BY forn_nome ')
    Params = <>
    Left = 368
    Top = 472
  end
  object dspForn: TDataSetProvider
    DataSet = zqryForn
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 408
    Top = 472
  end
  object cdsForn: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspForn'
    Left = 448
    Top = 472
    object cdsFornforn_codigo: TIntegerField
      FieldName = 'forn_codigo'
      Required = True
    end
    object cdsFornforn_nome: TWideStringField
      FieldName = 'forn_nome'
      Size = 80
    end
  end
  object dsForn: TDataSource
    DataSet = cdsForn
    Left = 488
    Top = 472
  end
  object zqryStatus: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      'SELECT codi_com, valo_com FROM tabela_combos')
    Params = <>
    Left = 368
    Top = 424
  end
  object dspStatus: TDataSetProvider
    DataSet = zqryStatus
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 408
    Top = 424
  end
  object cdsStatus: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspStatus'
    Left = 448
    Top = 424
    object LargeintField4: TLargeintField
      FieldName = 'codi_com'
      Required = True
    end
    object WideStringField4: TWideStringField
      FieldName = 'valo_com'
      Size = 50
    end
  end
  object dsStatus: TDataSource
    DataSet = cdsStatus
    Left = 488
    Top = 424
  end
  object zqryHistorico: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT his_codigo, rnc_codigo, his_data, his_historico, his_usua' +
        'rio, his_disposicao'
      '  FROM rnc_historico')
    Params = <>
    Left = 560
    Top = 232
  end
  object dspHistorico: TDataSetProvider
    DataSet = zqryHistorico
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 600
    Top = 232
  end
  object cdsHistorico: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspHistorico'
    Left = 640
    Top = 232
    object cdsHistoricohis_codigo: TIntegerField
      FieldName = 'his_codigo'
      Required = True
    end
    object cdsHistoricornc_codigo: TIntegerField
      FieldName = 'rnc_codigo'
      Required = True
    end
    object cdsHistoricohis_data: TDateTimeField
      FieldName = 'his_data'
    end
    object cdsHistoricohis_historico: TWideStringField
      FieldName = 'his_historico'
      Size = 250
    end
    object cdsHistoricohis_usuario: TWideStringField
      FieldName = 'his_usuario'
      Size = 30
    end
    object cdsHistoricohis_disposicao: TWideMemoField
      FieldName = 'his_disposicao'
      OnGetText = cdsHistoricohis_disposicaoGetText
      BlobType = ftWideMemo
    end
  end
  object dsHistorico: TDataSource
    DataSet = cdsHistorico
    Left = 680
    Top = 232
  end
end
