object FormCadCronograma: TFormCadCronograma
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Cronograma de Implementa'#231#227'o'
  ClientHeight = 509
  ClientWidth = 1068
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnl1: TPanel
    Left = 0
    Top = 437
    Width = 1068
    Height = 72
    Align = alBottom
    TabOrder = 0
    object pnlNavegacao: TPanel
      Left = 551
      Top = 20
      Width = 161
      Height = 40
      TabOrder = 0
      object btnPrimeiro: TBitBtn
        Left = 1
        Top = 1
        Width = 40
        Height = 38
        Hint = 'Primeiro registro'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = btnPrimeiroClick
      end
      object btnUltimo: TBitBtn
        Left = 121
        Top = 1
        Width = 40
        Height = 38
        Hint = #218'ltimo registro'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        OnClick = btnUltimoClick
      end
      object btnProximo: TBitBtn
        Left = 81
        Top = 1
        Width = 40
        Height = 38
        Hint = 'Pr'#243'ximo registro'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnProximoClick
      end
      object btnAnterior: TBitBtn
        Left = 41
        Top = 1
        Width = 40
        Height = 38
        Hint = 'Registro anterior'
        Align = alLeft
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        OnClick = btnAnteriorClick
      end
    end
    object btnSair: TBitBtn
      Left = 994
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para sair'
      Align = alRight
      Caption = 'Sair'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = btnSairClick
    end
    object btnImprimir: TBitBtn
      Left = 366
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para imprimir'
      Align = alLeft
      Caption = '&Imprimir'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = btnImprimirClick
    end
    object btnCancelar: TBitBtn
      Left = 293
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para cancelar a opera'#231#227'o'
      Align = alLeft
      Caption = '&Cancelar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = btnCancelarClick
    end
    object btnGravar: TBitBtn
      Left = 220
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para gravar o registro'
      Align = alLeft
      Caption = '&Gravar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnClick = btnGravarClick
    end
    object btnExcluir: TBitBtn
      Left = 147
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para excluir um registro'
      Align = alLeft
      Caption = '&Excluir'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnClick = btnExcluirClick
    end
    object btnAlterar: TBitBtn
      Left = 74
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para alterar um registro'
      Align = alLeft
      Caption = '&Alterar'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFDFDFDAEACAB757372817E7ED6D5D5FFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = btnAlterarClick
    end
    object btnNovo: TBitBtn
      Left = 1
      Top = 1
      Width = 73
      Height = 70
      Hint = 'Clique para adicionar um novo registro'
      Align = alLeft
      Caption = '&Novo'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 7
      OnClick = btnNovoClick
    end
    object btnExcel: TBitBtn
      Left = 439
      Top = 1
      Width = 95
      Height = 70
      Hint = 'Clique para exportar as informa'#231#245'es para o Excel'
      Align = alLeft
      Caption = '&Exportar Excel'
      DoubleBuffered = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        EFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentDoubleBuffered = False
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 8
      OnClick = btnExcelClick
    end
  end
  object pgcCrono: TPageControl
    Left = 0
    Top = 0
    Width = 1068
    Height = 437
    ActivePage = tsCadastro
    Align = alClient
    TabOrder = 1
    OnChange = pgcCronoChange
    object tsCadastro: TTabSheet
      Caption = 'Cadastro'
      object lbl4: TLabel
        Left = 7
        Top = 12
        Width = 44
        Height = 13
        Caption = 'Requisito'
      end
      object lbl14: TLabel
        Left = 135
        Top = 12
        Width = 166
        Height = 13
        Caption = 'Tela de acesso no Destra Manager'
      end
      object lbl34: TLabel
        Left = 516
        Top = 206
        Width = 65
        Height = 13
        Caption = 'Data Prevista'
      end
      object lbl6: TLabel
        Left = 628
        Top = 206
        Width = 72
        Height = 13
        Caption = 'Data Realizada'
      end
      object lbl8: TLabel
        Left = 7
        Top = 206
        Width = 61
        Height = 13
        Caption = 'Respons'#225'vel'
        Color = clBackground
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
        Transparent = True
      end
      object lbl1: TLabel
        Left = 7
        Top = 55
        Width = 45
        Height = 13
        Caption = 'Atividade'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object lbl2: TLabel
        Left = 739
        Top = 206
        Width = 31
        Height = 13
        Caption = 'Status'
      end
      object lbl3: TLabel
        Left = 7
        Top = 253
        Width = 100
        Height = 13
        Caption = 'Resultado Alcan'#231'ado'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object edtRequisito: TEdit
        Left = 7
        Top = 28
        Width = 122
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 30
        ParentFont = False
        TabOrder = 0
      end
      object edtTelaDestra: TEdit
        Left = 135
        Top = 28
        Width = 914
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
      end
      object dtPrevista: TDateEdit
        Left = 516
        Top = 222
        Width = 105
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 4
      end
      object dtRealizada: TDateEdit
        Left = 628
        Top = 222
        Width = 105
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        TabOrder = 5
      end
      object dblResponsavel: TDBLookupComboBox
        Left = 7
        Top = 222
        Width = 490
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_col'
        ListField = 'nome_col'
        ListSource = dsResp
        ParentFont = False
        TabOrder = 3
      end
      object mmoAtividade: TMemo
        Left = 7
        Top = 69
        Width = 1042
        Height = 133
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ScrollBars = ssVertical
        TabOrder = 2
        OnExit = mmoAtividadeExit
        OnKeyPress = mmoAtividadeKeyPress
      end
      object edtStatus: TEdit
        Tag = 30
        Left = 739
        Top = 222
        Width = 310
        Height = 26
        CharCase = ecUpperCase
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        MaxLength = 30
        ParentFont = False
        TabOrder = 6
      end
      object mmoResultado: TMemo
        Left = 7
        Top = 268
        Width = 1042
        Height = 136
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        ScrollBars = ssVertical
        TabOrder = 7
        OnExit = mmoResultadoExit
        OnKeyPress = mmoAtividadeKeyPress
      end
    end
    object tsPesquisa: TTabSheet
      Caption = 'Pesquisa'
      ImageIndex = 2
      object lbl9: TLabel
        Left = 3
        Top = 13
        Width = 42
        Height = 13
        Caption = 'Pesquisa'
      end
      object lbl10: TLabel
        Left = 327
        Top = 13
        Width = 65
        Height = 13
        Caption = 'Pesquisar por'
      end
      object edtValor: TEdit
        Tag = 30
        Left = 3
        Top = 32
        Width = 318
        Height = 26
        CharCase = ecUpperCase
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnChange = edtValorChange
      end
      object cbbCampo: TComboBox
        Tag = 30
        Left = 327
        Top = 32
        Width = 251
        Height = 26
        Style = csDropDownList
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ItemIndex = 1
        ParentFont = False
        TabOrder = 1
        Text = 'Atividade'
        Items.Strings = (
          'Requisito'
          'Atividade')
      end
      object dbgCronograma: TDBGrid
        Left = 3
        Top = 63
        Width = 1057
        Height = 344
        Hint = 'Selecione o registro para alterar'
        TabStop = False
        DataSource = dsCronograma
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnCellClick = dbgCronogramaCellClick
        OnDblClick = dbgCronogramaDblClick
        OnKeyUp = dbgCronogramaKeyUp
        Columns = <
          item
            Expanded = False
            FieldName = 'cro_requisito'
            Title.Caption = 'Requisito'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'atividade'
            Title.Caption = 'Atividade'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 350
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'responsavel'
            Title.Caption = 'Respons'#225'vel'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 220
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'cro_data_prevista'
            Title.Caption = 'Data Prevista'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 85
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'cro_data_realizada'
            Title.Caption = 'Data Realizada'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'status'
            Title.Caption = 'Status'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 170
            Visible = True
          end>
      end
    end
  end
  object pnlImprimir: TPanel
    Left = 485
    Top = 117
    Width = 380
    Height = 270
    TabOrder = 2
    object pnl2: TPanel
      Left = 1
      Top = 197
      Width = 378
      Height = 72
      Align = alBottom
      TabOrder = 1
      object btnVideo: TBitBtn
        Left = 1
        Top = 1
        Width = 73
        Height = 70
        Hint = 'Clique para imprimir'
        Align = alLeft
        Caption = '&V'#237'deo'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnClick = btnVideoClick
      end
      object btnImpressora: TBitBtn
        Left = 74
        Top = 1
        Width = 73
        Height = 70
        Hint = 'Clique para imprimir'
        Align = alLeft
        Caption = '&Impressora'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        OnClick = btnImpressoraClick
      end
      object btnSairImp: TBitBtn
        Left = 304
        Top = 1
        Width = 73
        Height = 70
        Hint = 'Clique para sair'
        Align = alRight
        Caption = 'Sair'
        DoubleBuffered = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        ParentDoubleBuffered = False
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnSairImpClick
      end
    end
    object pnl3: TPanel
      Left = 1
      Top = 1
      Width = 378
      Height = 29
      Align = alTop
      Alignment = taLeftJustify
      Caption = ' Filtro de Impress'#227'o'
      Color = clGradientActiveCaption
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object grpResp: TGroupBox
      Left = 17
      Top = 115
      Width = 353
      Height = 77
      Caption = 'Respons'#225'vel'
      TabOrder = 2
      object dblResp: TDBLookupComboBox
        Tag = 30
        Left = 4
        Top = 19
        Width = 346
        Height = 26
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'codi_col'
        ListField = 'nome_col'
        ListSource = dsResp
        ParentFont = False
        TabOrder = 0
      end
      object chkTodosResp: TCheckBox
        Tag = 30
        Left = 8
        Top = 51
        Width = 97
        Height = 17
        Caption = 'TODOS'
        TabOrder = 1
        OnClick = chkTodosRespClick
      end
    end
    object rgTipoRel: TRadioGroup
      Tag = 30
      Left = 17
      Top = 36
      Width = 353
      Height = 73
      Caption = 'Tipo de Relat'#243'rio'
      Columns = 2
      Items.Strings = (
        'Cronograma Geral'
        'Atividades Realizadas')
      TabOrder = 3
    end
  end
  object dbgExcel: TDBGrid
    Left = 695
    Top = 84
    Width = 204
    Height = 74
    DataSource = dsExcel
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    Visible = False
  end
  object zqryGravar: TZQuery
    Connection = dm.Conexao
    Params = <>
    Left = 440
    Top = 120
  end
  object dspGravar: TDataSetProvider
    DataSet = zqryGravar
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 480
    Top = 120
  end
  object cdsGravar: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspGravar'
    Left = 520
    Top = 120
  end
  object zqryImprimir: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT cro_codigo, cro_requisito, cro_tela_destra, cro_atividade' +
        ', cro_responsavel, '
      
        '       cro_data_prevista, cro_data_realizada, cro_status, cro_re' +
        'sultado,'
      '       C.nome_col as Responsavel,'
      
        '       CAST(CASE WHEN cro_data_prevista ISNULL THEN '#39'N'#195'O PLANEJA' +
        'DO'#39
      
        '            WHEN cro_data_prevista > '#39'2017-10-02'#39' AND cro_data_r' +
        'ealizada ISNULL THEN '#39'ATIVIDADE NO PRAZO'#39
      
        '            WHEN cro_data_prevista <= '#39'2017-10-02'#39' AND cro_data_' +
        'realizada ISNULL THEN '#39'ATIVIDADE FORA DO PRAZO'#39
      
        '            WHEN cro_data_realizada NOTNULL THEN '#39'ATIVIDADE REAL' +
        'IZADA'#39
      '       END as character varying(25)) as Status'
      'FROM cronograma'
      'LEFT JOIN Colaboradores C ON C.codi_col = cro_responsavel'
      'ORDER BY cro_codigo')
    Params = <>
    Left = 440
    Top = 168
  end
  object dspImprimir: TDataSetProvider
    DataSet = zqryImprimir
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 480
    Top = 168
  end
  object cdsImprimir: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspImprimir'
    Left = 520
    Top = 168
    object cdsImprimircro_codigo: TIntegerField
      FieldName = 'cro_codigo'
      Required = True
    end
    object cdsImprimircro_requisito: TWideStringField
      FieldName = 'cro_requisito'
      Required = True
    end
    object cdsImprimircro_tela_destra: TWideStringField
      FieldName = 'cro_tela_destra'
      Size = 200
    end
    object cdsImprimircro_atividade: TWideMemoField
      FieldName = 'cro_atividade'
      BlobType = ftWideMemo
    end
    object cdsImprimircro_responsavel: TIntegerField
      FieldName = 'cro_responsavel'
    end
    object cdsImprimircro_data_prevista: TDateTimeField
      FieldName = 'cro_data_prevista'
    end
    object cdsImprimircro_data_realizada: TDateTimeField
      FieldName = 'cro_data_realizada'
    end
    object cdsImprimircro_status: TWideStringField
      FieldName = 'cro_status'
    end
    object cdsImprimircro_resultado: TWideMemoField
      FieldName = 'cro_resultado'
      BlobType = ftWideMemo
    end
    object cdsImprimirresponsavel: TWideStringField
      FieldName = 'responsavel'
      ReadOnly = True
      Size = 45
    end
    object cdsImprimirstatus: TWideStringField
      FieldName = 'status'
      ReadOnly = True
      Size = 25
    end
  end
  object zqryCronograma: TZQuery
    Connection = dm.Conexao
    Params = <>
    Left = 48
    Top = 176
  end
  object dspCronograma: TDataSetProvider
    DataSet = zqryCronograma
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 88
    Top = 176
  end
  object cdsCronograma: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspCronograma'
    Left = 128
    Top = 176
  end
  object dsCronograma: TDataSource
    DataSet = cdsCronograma
    Left = 168
    Top = 176
  end
  object frxReport1: TfrxReport
    Version = '4.10.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40548.430550960600000000
    ReportOptions.LastChange = 43304.821542442130000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      
        'procedure mConfidencialCabecOnBeforePrint(Sender: TfrxComponent)' +
        ';'
      'begin'
      '   if <ConfCabec> = 1 then begin'
      
        '      mConfidencialCabec.Visible:= True;                        ' +
        '               '
      '   end'
      '   else begin'
      '      mConfidencialCabec.Visible:= False;  '
      '   end;  '
      'end;'
      ''
      
        'procedure mConfidencialRodapeOnBeforePrint(Sender: TfrxComponent' +
        ');'
      'begin'
      '   if <ConfRod> = 1 then begin'
      
        '      mConfidencialRodape.Visible:= True;                       ' +
        '                '
      '   end'
      '   else begin'
      '      mConfidencialRodape.Visible:= False;  '
      '   end;       '
      'end;'
      '  '
      'begin'
      ''
      'end.')
    Left = 272
    Top = 168
    Datasets = <
      item
        DataSet = frxDBCronograma
        DataSetName = 'frxDBCronograma'
      end>
    Variables = <
      item
        Name = ' New Category1'
        Value = Null
      end
      item
        Name = 'vlMedia'
        Value = Null
      end
      item
        Name = ' Texto Relat'#243'rio'
        Value = Null
      end
      item
        Name = 'ConfCabec'
        Value = ''
      end
      item
        Name = 'ConfRod'
        Value = ''
      end
      item
        Name = 'TextoConf'
        Value = ''
      end>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 297.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      object ReportTitle1: TfrxReportTitle
        Height = 101.000012200000000000
        Top = 18.897650000000000000
        Width = 1046.929810000000000000
        StartNewPage = True
        object Memo4: TfrxMemoView
          Top = 18.488250000000000000
          Width = 1046.929810000000000000
          Height = 34.015770000000000000
          ShowHint = False
          AutoWidth = True
          DataSetName = 'cdsParametroRelatorio'
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -21
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'CRONOGRAMA - ATIVIDADES REALIZADAS')
          ParentFont = False
        end
        object Line2: TfrxLineView
          Width = 1046.929810000000000000
          ShowHint = False
          Diagonal = True
        end
        object Memo6: TfrxMemoView
          Top = 81.913420000000000000
          Width = 76.346439610000100000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Requisito')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo9: TfrxMemoView
          Left = 75.826840000000000000
          Top = 82.102362200000000000
          Width = 385.134063310000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Atividade')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo2: TfrxMemoView
          Left = 461.102660000000000000
          Top = 81.992125980000000000
          Width = 222.614273310000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Respons'#225'vel')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo5: TfrxMemoView
          Left = 684.094930000000000000
          Top = 81.992125980000000000
          Width = 90.330723310000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Data Realizada')
          ParentFont = False
          VAlign = vaBottom
        end
        object Memo12: TfrxMemoView
          Left = 773.953310000000000000
          Top = 81.992125980000000000
          Width = 271.748163310000000000
          Height = 18.897650000000000000
          ShowHint = False
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Resultados Alcan'#231'ados')
          ParentFont = False
          VAlign = vaBottom
        end
        object mConfidencialCabec: TfrxMemoView
          Left = 3.779530000000000000
          Width = 982.677165350000000000
          Height = 18.897650000000000000
          OnBeforePrint = 'mConfidencialCabecOnBeforePrint'
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[TextoConf]')
          ParentFont = False
        end
      end
      object Detalhe: TfrxMasterData
        Height = 90.708720000000000000
        Top = 181.417440000000000000
        Width = 1046.929810000000000000
        DataSet = frxDBCronograma
        DataSetName = 'frxDBCronograma'
        RowCount = 0
        object frxControleDATA: TfrxMemoView
          Width = 76.346439610000100000
          Height = 90.708720000000000000
          ShowHint = False
          DataField = 'cro_requisito'
          DataSet = frxDBCronograma
          DataSetName = 'frxDBCronograma'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBCronograma."cro_requisito"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          Left = 75.826840000000000000
          Width = 385.134063310000000000
          Height = 90.708720000000000000
          ShowHint = False
          DataField = 'cro_atividade'
          DataSet = frxDBCronograma
          DataSetName = 'frxDBCronograma'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBCronograma."cro_atividade"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo3: TfrxMemoView
          Left = 461.102660000000000000
          Width = 222.614273310000000000
          Height = 90.708720000000000000
          ShowHint = False
          DataField = 'responsavel'
          DataSet = frxDBCronograma
          DataSetName = 'frxDBCronograma'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBCronograma."responsavel"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          Left = 684.094930000000000000
          Width = 90.330723310000000000
          Height = 90.708720000000000000
          ShowHint = False
          DataField = 'cro_data_realizada'
          DataSet = frxDBCronograma
          DataSetName = 'frxDBCronograma'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBCronograma."cro_data_realizada"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          Left = 773.953310000000000000
          Width = 271.748163310000000000
          Height = 90.708720000000000000
          ShowHint = False
          DataField = 'cro_resultado'
          DataSet = frxDBCronograma
          DataSetName = 'frxDBCronograma'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBCronograma."cro_resultado"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object PageFooter1: TfrxPageFooter
        Height = 52.913420000000000000
        Top = 332.598640000000000000
        Width = 1046.929810000000000000
        object Picture2: TfrxPictureView
          Left = 993.795920000000000000
          Width = 37.795300000000000000
          Height = 34.015770000000000000
          ShowHint = False
          Picture.Data = {
            
            D9}
          Stretched = False
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo1: TfrxMemoView
          Left = 3.779530000000000000
          Top = 30.236240000000010000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'P'#225'gina: [Page#] de [TotalPages#]')
          ParentFont = False
        end
        object Line1: TfrxLineView
          Left = 3.779530000000000000
          Top = 26.456709999999990000
          Width = 975.118124880000000000
          ShowHint = False
          Frame.Typ = [ftTop]
        end
        object mConfidencialRodape: TfrxMemoView
          Left = 3.779530000000000000
          Top = 7.559059999999988000
          Width = 982.677165350000000000
          Height = 18.897650000000000000
          OnBeforePrint = 'mConfidencialRodapeOnBeforePrint'
          ShowHint = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            '[TextoConf]')
          ParentFont = False
        end
      end
    end
  end
  object frxDBCronograma: TfrxDBDataset
    UserName = 'frxDBCronograma'
    CloseDataSource = False
    FieldAliases.Strings = (
      'cro_codigo=cro_codigo'
      'cro_requisito=cro_requisito'
      'cro_tela_destra=cro_tela_destra'
      'cro_atividade=cro_atividade'
      'cro_responsavel=cro_responsavel'
      'cro_data_prevista=cro_data_prevista'
      'cro_data_realizada=cro_data_realizada'
      'cro_status=cro_status'
      'cro_resultado=cro_resultado'
      'responsavel=responsavel'
      'status=status')
    DataSet = cdsImprimir
    BCDToCurrency = False
    Left = 352
    Top = 168
  end
  object zqryResp: TZQuery
    Connection = dm.Conexao
    Params = <>
    Left = 248
    Top = 296
  end
  object dspResp: TDataSetProvider
    DataSet = zqryResp
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 288
    Top = 296
  end
  object cdsResp: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspResp'
    Left = 328
    Top = 296
  end
  object dsResp: TDataSource
    DataSet = cdsResp
    Left = 368
    Top = 296
  end
  object ImageList: TImageList
    Left = 640
    Top = 184
    Bitmap = {
      
      000000000000}
  end
  object zqryExcel: TZQuery
    Connection = dm.Conexao
    SQL.Strings = (
      
        'SELECT lm.iden_lis as "Identifica'#231#227'o", lm.revi_lis as "Revis'#227'o",' +
        ' lm.desc_lis as "Descri'#231#227'o",  '
      
        'CASE WHEN lm.tipo_lis = 0 THEN '#39'INTERNO'#39' ELSE '#39'EXTERNO'#39' END as "' +
        'Tipo do Documento", lm.atua_lis as "Atualiza'#231#227'o", '
      
        'F.desc_fun as "Respons'#225'vel", CASE WHEN apro_lis = '#39'S'#39' THEN '#39'SIM'#39 +
        ' ELSE '#39'N'#195'O'#39' END as "Aprovado", '
      'p.nome_pro as "Processo", lmp.lis_qtd as "Quantidade"'
      'FROM lista_mestra lm'
      
        'LEFT JOIN lista_mestra_proc lmp ON lmp.lis_codiDocumento = lm.co' +
        'di_lis'
      'LEFT JOIN processos P ON P.codi_pro = lmp.lis_codiProcesso'
      'LEFT JOIN funcoes F ON F.codi_fun = lm.resp_lis'
      '')
    Params = <>
    Left = 696
    Top = 40
  end
  object dspExcel: TDataSetProvider
    DataSet = zqryExcel
    Options = [poAutoRefresh, poAllowCommandText]
    UpdateMode = upWhereKeyOnly
    Left = 736
    Top = 40
  end
  object cdsExcel: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspExcel'
    Left = 776
    Top = 40
  end
  object dsExcel: TDataSource
    DataSet = cdsExcel
    Left = 816
    Top = 40
  end
end
